...
class MY_Migration extends CI_Migration
{
	...
	public function execute_sql_mysql_client($sql_file)
	{
		...
		// 2nd: Default paths
		$mysql_paths[] = 'C:\Program Files\PHP App Sales Manager Stack\mysql\bin\mysql.exe';  //Windows
		$mysql_paths[] = 'C:\PHPPOS\mysql\bin\mysql.exe';  //Windows
		$mysql_paths[] = '/Applications/phppos/mysql/bin/mysql';  //Mac
		$mysql_paths[] = '/Applications/MAMP/Library/bin/mysql';  //Mac Mamp
		$mysql_paths[] = 'c:\xampp\mysql\bin\mysql.exe';//XAMPP

		$mysql_paths[] = '/opt/phppos/mysql/bin/mysql';  //Linux
		...
		foreach($mysql_paths as $mysql_path)
		{
			if (is_executable($mysql_path))
			{
				
				$command_path = "\"$mysql_path\" --host=$db_hostname --user=$db_username --password=$db_password $database < \"$sql_file\"";
				exec($command_path);
				return;
			}
		}		
	...
	public function execute_sql($file)
	{
		...
		{
			mysqli_query($this->db->conn_id,'SET SESSION innodb_strict_mode=OFF');
		...
		else
		{
			...
			if ($mysqlnd && version_compare(PHP_VERSION, '5.3.0') >= 0) 
			{
				...
				$sql = 'SET SESSION innodb_strict_mode=OFF';
				$db->exec($sql);
