...
class Locations extends Secure_area implements Idata_controller
{
	...
	*/
	...
	{
		$branding = $this->config->item('branding');
		$domain = $branding['domain'];
		$staging_domain = $branding['staging_domain'];
		
		$current_location_count = $this->Location->count_all();
		$auth_url = (!defined("ENVIRONMENT") or ENVIRONMENT == 'development') ? "http://$staging_domain/allowed_stores.php?email=".rawurlencode($email): "http://$domain/allowed_stores.php?email=".rawurlencode($email);
