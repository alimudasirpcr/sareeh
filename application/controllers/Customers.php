...
class Customers extends Person_controller
{
	...
	{
		$params = $this->session->userdata('customers_search_data') ? $this->session->userdata('customers_search_data') : array('offset' => 0, 'order_col' => 'last_name', 'order_dir' => 'asc', 'search' => FALSE, 'deleted' => 0, 'location_id' => $this->config->item('only_allow_current_location_customers') ? $this->Employee->get_logged_in_employee_current_location_id() : '');
	...
	function sorting()
	{
		$params = $this->session->userdata('customers_search_data') ? $this->session->userdata('customers_search_data') : array('order_col' => 'last_name', 'order_dir' => 'asc','deleted' => 0,'location_id' => $this->config->item('only_allow_current_location_customers') ? $this->Employee->get_logged_in_employee_current_location_id() : '');
	...
	*/
	function view($customer_id=-1,$redirect_code=0)
	{
		...
		$redirect_new_order = $this->input->get('redirect_new_order');
	...
	{
		$params = $this->session->userdata('customers_search_data');
		$this->session->set_userdata('customers_search_data', array('offset' => 0, 'order_col' => 'last_name', 'order_dir' => 'asc', 'search' => FALSE, 'deleted' => $params['deleted'],'location_id' => $this->config->item('only_allow_current_location_customers') ? $this->Employee->get_logged_in_employee_current_location_id() : ''));
		redirect('customers');
	}
	/*
	...
	*/
	...
	function excel_export() {
		...
		foreach ($data as $r) {
			...
			$row[] = $r->default_term_id ? $this->Invoice->get_term($r->default_term_id)->name : NULL;
		...
		array_to_spreadsheet($rows,'customers_export.'.$extension, FALSE);
	}
	...
	function complete_excel_import()
	{
		...
		for ($i = 0; $i < $numRows -1; $i++)
		{
			...
			if(!$this->Customer->save_customer($person_data,$customer_data, $person_id ? $person_id : FALSE, false))
			{	
				if($person_id === FALSE)
				{
	...
	{	
		...
		if($key == 'default_term_id'){
			if(!$value)
			{
				 return NULL;
			}

			$term = $this->Invoice->get_term_by_name($value)->term_id;
	...
	function export_to_sidekick(){
		...
		$number_of_customers_completed = 0;
		$this->Appconfig->save('percentage_of_customer_export_to_sidekick', 0);
		while ($customer = $customers->unbuffered_row())
		{
			if($customer->email || $customer->phone_number){
				...
				if($customer->location_id){
					$customer_location = $this->Location->get_info($customer->location_id);
					$headers = array(
			...
			$number_of_customers_completed++;
			$percent_complete = floor(($number_of_customers_completed/$total_customer)*100);
			$this->Appconfig->save('percentage_of_customer_export_to_sidekick', $percent_complete);
		}

		echo json_encode(array('success'=>true,'message'=>lang('customers_export_to_sidekick_successful')));
	}

	function get_completed_percentage(){
		session_write_close();
		echo json_encode(array("percent_complete" => $this->Appconfig->get('percentage_of_customer_export_to_sidekick')));
	}
