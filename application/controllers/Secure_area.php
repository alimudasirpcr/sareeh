	...
	*/
	...
	;
			...
			if($query_string){
				...
				redirect('login?continue='.rawurlencode(uri_string()));
			}
			
		}else{
			$employee = $this->Employee->get_info($this->session->userdata('person_id'));

			if($employee->inactive == 1 || $employee->deleted == 1){
		...
		$data['disable_modules'] = $this->config->item('disable_modules') ? unserialize($this->config->item('disable_modules')) : false;
