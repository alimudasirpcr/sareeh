...
class Employees extends Person_controller
{
	...
	function sorting()
	{
		...
		else
		{
			...
			$table_data = $this->Employee->get_all($deleted,$per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $order_col, $order_dir, true);
		}
		$config['base_url'] = site_url('employees/sorting');
	...
	*/
	...
	function edit_profile()
	{
		...
		$this->load->view('employees/edit_profile', $data);
	...
	function setup_2fa(){
		...
		$company = $this->Location->get_info_for_key('company')?:$this->config->item('company');
		$email = $this->Employee->get_logged_in_employee_info()->email;

		$data['secret_key'] = $secretKey;
		$data['qr_code'] = $google2fa->getQRCodeInline($company, $email, $secretKey,220);

		$this->load->view('employees/setup_2fa', $data);
	}
	...
	function disable_2fa(){
		...
		$secret_key = $employee_info->secret_key_2fa;
		$security_code = $this->input->post('security_code_to_disable');
		$isValid  = $google2fa->verifyKey($secret_key, $security_code);
	...
	Loads the employee edit form
	*/
	function edit_profile_model($employee_id=-1,$redirect_code=0)
	{
 	 	if (!$this->Employee->has_module_action_permission('employees', 'edit_profile', $this->Employee->get_logged_in_employee_info()->person_id))
		{
			redirect('no_access/home');
		...
		$email 						= 	$this->Employee->get_logged_in_employee_info()->email;
		$data['secret_key'] 		= 	$secretKey;
		$data['qr_code'] 			= 	$google2fa->getQRCodeInline($company, $email, $secretKey,100);


		$this->load->view('employees/edit_profile_model', $data);
	}

	function quick_modal()
	{
		$data['controller_name']	=	strtolower(get_class());
		$data['title'] 				= 	lang('employees_new');
