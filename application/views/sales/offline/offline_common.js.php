    ...
    function update_cart_ui() {
        ...
        cart.forEach(function(item, index) {
            ...
            // console.log(`ID: ${item.id}, Price: ${item.price}, Quantity: ${item.qty}`);
                ...
                $('#quantity_' + index).prepend('<button onclick="inc_de_qty('+ index+', -1)" type="button" class="btn w-25px h-25px  btn-icon rounded-circle btn-light"><i class="bi bi-dash fs-1"></i></button>');
                $('#quantity_' + index).append( '<button onclick="inc_de_qty('+ index+', 1)" type="button" class="btn w-25px h-25px  btn-icon rounded-circle btn-light"> <i class="bi bi-plus fs-1"></i></button>');
