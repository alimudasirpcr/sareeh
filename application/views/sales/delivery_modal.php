	...
	<div class="modal-content">
		...
		<div class="modal-body" id="myTabModalBody">
									...
									<div class="form-group row">
										...
										<div class="col-sm-9 col-md-9 col-lg-9">
												...
												array(
													...
													'id' => 'tracking_number',
													'class' => 'form-control',
													'value' => $delivery_info['tracking_number'] ? $delivery_info['tracking_number'] : ($delivery_info['sale_id'] ? ($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS').$delivery_info['sale_id'] : ( $this->cart->get_previous_receipt_id() ?  ($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS').$this->cart->get_previous_receipt_id() : '') )
												)
											); ?>
										</div>
						 			</div>	
		         ...
		         <div class="tab-pane" id="ShippingInfo">
							...
							<div class="form-group row">
								<label class="col-sm-3 col-md-3 col-lg-3 control-label"><?php echo lang("deliveries_contact_preference")?> :</label>										
								<div class="col-sm-9 col-md-9 col-lg-9">
									<ul class="list-inline">
										<?php foreach($contact_preference as $preference_key => $preference_value){ ?>
											<li>
												<input type="checkbox" name="contact_preference[]" id="contact_preference_<?php echo $preference_key; ?>" value="<?php echo $preference_value; ?>" <?php echo in_array($preference_value, isset($delivery_info['contact_preference']) ? $delivery_info['contact_preference'] : array()) ? "checked" : ""; ?>>
...
<script type='text/javascript'>
	...
	$(document).ready(function()
	{
			...
			function zip_change() 
			{
				...
				for(var zip_key in zip_zones)
				{
					var zone = zip_zones[zip_key];
					var search = new RegExp(zip_key, 'g');
					if (zip.match(search))
					{
						$('#shipping_zone_id').val(zone).trigger('change');
						break;
					}
				}
			}
	...
	function get_delivery_info() {
		...
		var delivery_info = {
			...
			delivery_employee_person_id: $("#delivery_employee_person_id").val(),
		};
		...
		var checked_contact_preference = [];
		$("input[name='contact_preference[]']:checked").each(function (){
			checked_contact_preference.push($(this).val());
		});

		delivery_info['contact_preference'] = checked_contact_preference;

		delivery_info['location_id'] = $('#location_id').val();
