			...
			<div class="register-box register-items-form">
				...
				<div id="itemForm" class="item-form">
					...
					<div class="input-group contacts register-input-group">
						...
						<input type="text" id="item" name="item" <?php echo ($mode == "store_account_payment" || $mode == 'purchase_points') ? 'disabled="disabled"' : '' ?> class="add-item-input pull-left keyboardTop" placeholder="<?php echo lang('start_typing_item_name'); ?>" data-title="<?php echo lang('item_name'); ?>">
										...
										<tr class="register-item-bottom">
											...
											<td colspan="5">
												<dl class="register-item-extra-details dl-horizontal">
													...
													<?php if (property_exists($item,'is_recurring') && $item->is_recurring) { ?>
														<dt><?php echo lang('recurring_amount'); ?></dt>
													...
													<?php if ($cart->get_previous_receipt_id()) { ?>
														<dt><?php echo lang('qty_picked_up'); ?></dt>
													...
													if (property_exists($item, 'quantity_units') && count($item->quantity_units) > 0) { ?>
														...
														<dd class="">
															...
															<a href="#" id="quantity_unit_<?php echo $line; ?>" data-name="quantity_unit_id" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item/' . $line); ?>" data-title="<?php echo H(lang('quantity_units')); ?>"><?php echo character_limiter(H($item->quantity_unit_id ? $item->quantity_units[$item->quantity_unit_id] : lang('none')), 50); ?></a></dd>
														<?php
														$source_data = array();
													...
													?>
														<dt><?php echo lang('cost_price'); ?></dt>
														<dd>
													...
													if (!empty($variation_choices)) { ?>
														...
														<dd class=""><a href="#" id="variation_<?php echo $line; ?>" data-name="variation" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item_variation/' . $line); ?>" data-title="<?php echo H(lang('variation')); ?>"><?php echo character_limiter(H($item->variation_name), 50); ?></a></dd>
												...
												<dd class=""><a href="#" id="supplier_<?php echo $line; ?>" data-name="supplier" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item_supplier/' . $line); ?>" data-title="<?php echo H(lang('supplier')); ?>"><?php echo character_limiter(H($supplier_name), 50); ?></a></dd>
											...
											<?php } ?>
													...
													<?php
													if (count($tiers) > 1) { ?>
														<dt class=""><?php echo lang('tier'); ?> </dt>
														<dd class="">

															<?php if ($item->allow_price_override_regardless_of_permissions || $this->Employee->has_module_action_permission('sales', 'edit_sale_price', $this->Employee->get_logged_in_employee_info()->person_id)) {	?>
																<a href="#" id="tier_<?php echo $line; ?>" data-name="tier_id" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item/' . $line); ?>" data-title="<?php echo H(lang('tier')); ?>"><?php echo character_limiter(H($item->tier_id ? $item->tier_name : $tiers[$selected_tier_id]), 50); ?></a></dd>
													<?php } else { ?>
												...
												<?php if (!$this->config->item('hide_description_on_sales_and_recv')) { ?>
													...
													<?php if (isset($item->is_serialized) && $item->is_serialized == 1  && $item->name != lang('giftcard')) { ?>
														...
														?>
															<dd class=""><a href="#" id="serialnumber_<?php echo $line; ?>" data-name="serialnumber" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item/' . $line); ?>" data-title="<?php echo H(lang('serial_number')); ?>"><?php echo character_limiter(H($item->serialnumber), 50); ?></a></dd>
														<?php
														} else {
														?>
															<dd class="">
													...
													?>
														...
														if ($item->is_series_package) { ?>
															<dt><?php echo lang('series_quantity'); ?></dt>
															<dd><?php echo to_quantity($item->series_quantity); ?></dd>

															<dt><?php echo lang('series_days_to_use_within'); ?></dt>
		...
		<div class="sale-buttons">
			...
			<?php if (count($cart_items) > 0) { ?>
				...
				<a href="#" class="btn btn-<?php echo $this->cart->get_previous_receipt_id() ||  $this->cart->suspended ? 'suspended' : 'cancel'; ?>" id="cancel_sale_button">
					...
					<?php echo $this->cart->get_previous_receipt_id() ||  $this->cart->suspended ? lang('cancel_edit') : lang('sales_cancel_sale'); ?>
				</a>
		...
		<?php if (isset($customer)) { ?>
			...
			<div class="customer-badge">
				...
				<div class="details">
					...
					{
					?>
						<?php echo anchor("customers/quick_modal/$customer_id/1", '<i class="ion-ios-compose-outline"></i>',  array('id' => 'edit_customer', 'data-toggle'=>"modal", 'data-target'=>"#myModalDisableClose", 'class' => 'btn btn-edit btn-primary pull-right', 'title' => lang('update_customer'))) . ''; ?>
					...
					{
					?>
						<?php echo anchor("customers/view/$customer_id/1", '<i class="ion-ios-compose-outline"></i>',  array('id' => 'edit_customer', 'class' => 'btn btn-edit btn-primary pull-right', 'title' => lang('update_customer'))) . ''; ?>
					<?php
					}
					?>
			...
			<div class="customer-action-buttons  btn-group btn-group-justified ">
				...
				<?php echo '' . anchor("sales/delete_customer", '<i class="ion-close-circled"></i> ' . lang('detach'), array('id' => 'delete_customer', 'class' => 'btn')); ?>
			</div>
		...
		<?php } else {  ?>
			...
			<div class="customer-form">
				...
				<div class="input-group contacts">
					...
					<input type="text" id="customer" name="customer" class="add-customer-input keyboardLeft" data-title="<?php echo lang('customer_name'); ?>" placeholder="<?php echo lang('sales_start_typing_customer_name') . ($this->config->item('require_customer_for_sale') ? ' (' . lang('required') . ')' : ''); ?>">
				</div>
				</form>
	...
	<div class="register-box register-summary paper-cut">
		...
		<ul class="list-group">
			<?php if ($this->Employee->has_module_action_permission('sales', 'give_discount', $this->Employee->get_logged_in_employee_info()->person_id) && $mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
				...
				<?php if ($has_discount) { ?>
					...
					<li class="list-group-item global-discount-group" id="discount-reason-group">
						<div class="key"><?php echo lang('discount_reason') . ': '; ?> </div>
						<div class="value pull-right">
							<a href="#" id="discount_reason" class="xeditable" data-validate-number="false" data-placement="<?php echo $discount_editable_placement; ?>" data-type="textarea" data-pk="1" data-name="discount_reason" data-url="<?php echo site_url('sales/discount_reason'); ?>" data-title="<?php echo H(lang('discount_reason')); ?>" data-emptytext="<?php echo H(lang('comments')); ?>" data-placeholder="<?php echo H(lang('discount_reason')); ?>"><?php echo isset($discount_reason) &&  $discount_reason ? $discount_reason : ''; ?></a>
						</div>
					</li>
				<?php } ?>
			...
			<li class="sub-total list-group-item">
				...
				<?php if ($this->Employee->has_module_action_permission('sales', 'edit_taxes', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
					[<a href="<?php echo site_url('sales/edit_taxes/') ?>" class="" id="edit_taxes" data-toggle="modal" data-target="#myModal"><?php echo lang('edit_taxes'); ?></a>]
				<?php } ?>
			</li>
		...
		<?php if (count($cart_items) > 0) { ?>
					...
					<div class="row">
						<div id="create_work_order_holder" class="create_work_order_holder col-md-6">
							...
							<?php echo form_checkbox(array(
								...
								'value'=>'1',
								'checked'=>(boolean)$cart->create_work_order,
								));?>
			...
			<div class="comment-block">
				...
				<?php echo form_textarea(array('name' => 'comment', 'id' => 'comment', 'value' => $comment, 'rows' => '2', 'class' => 'form-control', 'data-title' => lang('comments'), 'placeholder' => lang('comments'))); ?>
				<?php echo form_checkbox(array(
			...
			if ((is_all_sale_credit_card_payments_confirmed($cart) && count($payments) > 0) || (count($payments) > 0 && !is_sale_integrated_cc_processing($cart) && !is_sale_integrated_ebt_sale($cart)  && !is_sale_integrated_giftcard_processing($cart))) { ?>
				<div id="finish_sale" class="finish-sale">
					...
					echo '<label for="prompt_for_card"><span></span>' . lang('prompt_for_card') . '</label>';
					...
					if ($cc_processor_class_name == 'CORECLEARBLOCKCHYPPROCESSOR' && $this->Location->get_info_for_key('blockchyp_terms_and_conditions'))
					{
						...
						echo '<label for="show_terms_and_conditions"><span></span>' . lang('show_terms_and_conditions') . '</label>';
					}
					echo form_close();
					?>
				</div>
			...
			<?php } else { ?>
				<div id="finish_sale" class="finish-sale">
					<?php echo form_open("sales/start_cc_processing?provider=" . rawurlencode($this->Location->get_info_for_key('credit_card_processor') ? $this->Location->get_info_for_key('credit_card_processor') : ''), array('id' => 'finish_sale_form', 'autocomplete' => 'off')); ?>
					<?php
...
<script type="text/javascript" language="javascript">
	...
	$(document).ready(function() {
		...
		{
			$.post('<?php echo site_url("sales/set_create_invoice");?>', {create_invoice:$('#create_invoice').is(':checked') ? '1' : '0'});
		});
		...
		{
			$.post('<?php echo site_url("sales/set_create_work_order");?>', {create_work_order:$('#create_work_order').is(':checked') ? '1' : '0'});
		});
