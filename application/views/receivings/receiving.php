					...
					<table id="register" class="table table-hover">
						...
						<thead>
							<tr class="register-items-header">
								...
								<th class="item_sort_able   item_name_heading w-25 <?php echo $this->cart->sort_column && $this->cart->sort_column == 'name'? ($this->cart->sort_type=='asc'?"ion-arrow-down-b":"ion-arrow-up-b"):"";?>"><?php echo lang('sales_item_name'); ?></th>
					...
					?>
						<tr class="<?php echo $row_class; ?>">
							...
							<td class="sp_pay text-center">
								...
								<button type="submit" class="btn <?php echo $btn_class; ?> pay_store_account_receiving"><?php echo isset($unpaid_receiving['paid']) && $unpaid_receiving['paid'] == TRUE  ? lang('remove_payment') : lang('pay'); ?></button>
								</form>
							</td>
						</tr>
				<?php
		...
		<?php if ($mode == "transfer") { ?>
			...
			<?php if (isset($location_from)) {  ?>
				...
				<?php if ($this->Employee->has_module_action_permission('receivings', 'allow_detach_from_edit', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
					...
					<?php echo '' . anchor("receivings/delete_location_from", '<i class="ion-close-circled"></i> ' . lang('detach'), array('id' => 'delete_location_from', 'class' => 'btn')); ?>
					<?php } ?>
				</div>
			...
			<?php } else {  ?>
				...
				<div class="customer-form">
					...
					<div class="input-group contacts d-flex">
						...
						<input type="text" id="location_from" name="location_from" class="add-customer-input w-75" placeholder="<?php echo lang('receivings_start_typing_location_name_from'); ?>" data-title="<?php echo lang('location'); ?>" />
			...
			<?php if (isset($location)) {  ?>
				...
				<?php if ($this->Employee->has_module_action_permission('receivings', 'allow_detach_to_edit', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
					...
					<?php echo '' . anchor("receivings/delete_location", '<i class="ion-close-circled"></i> ' . lang('detach'), array('id' => 'delete_location', 'class' => 'btn')); ?>
					<?php } ?>
				</div>
			<?php } else {  ?>
				...
				<div class="customer-form">
					...
					<div class="input-group contacts d-flex">
						...
						<input type="text" id="location" name="location" class="add-customer-input w-75" placeholder="<?php echo lang('receivings_start_typing_location_name'); ?>" data-title="<?php echo lang('location'); ?>" />
		...
		<ul class="list-group">
			...
			<li class="sub-total list-group-item receivings <?php echo (!$see_cost_price) ? "hide" : ""; ?>">
				...
				<?php if ($this->Employee->has_module_action_permission('receivings', 'edit_taxes', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
					...
					<?php if ($this->config->item('charge_tax_on_recv')) { ?>
						[<a href="<?php echo site_url('receivings/edit_taxes/') ?>" class="" id="edit_taxes" data-toggle="modal" data-target="#myModal"><?php echo lang('edit_taxes'); ?></a>]
					<?php } ?>
				<?php } ?>
			</li>
			...
			if ($mode != "transfer") { ?>
				<?php if ($supplier_required_check) { ?>
					<div class="add-payment">
						...
						<div class="input-group add-payment-form">
							...
							<div class="d-flex">
								<?php echo form_input(array('name' => 'amount_tendered', 'id' => 'amount_tendered', 'value' => to_currency_no_money($amount_due), 'class' => 'add-input numKeyboard form-control '.(!$has_cost_price_permission ? 'hidden' : ''), 'data-title' => lang('payment_amount')));	?>
				...
				?>
					<div class="custom_field_block">
						<?php echo form_label(lang('shipping_cost'), "shipping_cost", array('class' => 'control-label ')); ?>
			...
			<?php } ?>
							...
							<div class="custom_field_block <?php echo "custom_field_${k}_value"; ?>">
								...
								<?php } elseif ($this->Receiving->get_custom_field($k, 'type') == 'image' || $this->Receiving->get_custom_field($k, 'type') == 'file') {
									...
									if ($cart->{"custom_field_${k}_value"} && $this->Receiving->get_custom_field($k, 'type') == 'image') {
										...
										echo "<div class='delete-custom-image-recv'><a href='" . site_url('receivings/delete_custom_field_value/' . $k) . "'>" . lang('delete') . "</a></div>";
					...
					<div class="change-date">
						...
						<div class="comment-block">
							<div class="side-heading"><label id="comment_label" for="comment"><?php echo lang('comments'); ?> : </label></div>
							<?php echo form_textarea(array('name' => 'comment', 'id' => 'comment', 'value' => $comment, 'rows' => '2', 'class' => 'form-control', 'data-title' => lang('comments'))); ?>
						</div>
...
<div class="modal fade" id="var_popup" role="dialog" aria-hidden="true">
	<div class="modal-dialog qty-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label=<?php echo json_encode(lang('close')); ?>><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title"><?php echo lang('variation'); ?> <span id="var-customize"></span></h4>
			</div>
			<form id="save-qty-form" method="POST" action="<?php echo site_url('receivings/add_variations_qty'); ?>">
				<div class="modal-body clearfix">
					<div class="placeholder_attribute_vals pull-left variations-qty">
						<table style="width: 100%" class="table table-hover variation-qty-table">
							<thead>
								<tr class="register-items-header">
									<th><?php echo H(lang('variation')); ?></th>
									<th><?php echo H(lang('quantity')); ?></th>
								</tr>
							</thead>
						</table>
					</div>
					<div class="placeholder_supplier_vals">
						<table style="width: 100%" class="table table-hover secondary-supplier-table">
							<thead>
								<tr>
									<th></th>
									<th><?php echo H(lang('secondary_suppliers')); ?></th>
				...
				<div class="modal-footer">
					<button type="submit" class="btn btn-primary save-qty"><?php echo lang('save'); ?></button>
				</div>
			</form>
		</div>
	</div>
