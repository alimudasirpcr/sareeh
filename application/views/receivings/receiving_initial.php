	...
	<div id="sale-grid-big-wrapper" class="clearfix register <?php echo $this->config->item('hide_images_in_grid') ? 'hide_images' : ''; ?>"">
		<div class="clearfix" id="category_item_selection_wrapper">
			<div class="">
				...
				<div id="grid_breadcrumbs" class="px-4"></div>
				<div id="category_item_selection" class="row register-grid nav nav-pills nav-pills-custom  p-0 mt-1  mx-0 mb-4"></div>
				<div id="category_item_selection_wrapper_new" class="row register-grid nav nav-pills nav-pills-custom  p-0 mt-1  mx-0 mb-4"></div>
	...
	$(document).ready(function() {
		...
		function processTagsResult(json) {
			...
			for (var k = 0; k < json.tags.length; k++) {
				...
				var tag_item = '<li data-tag_id="'+json.tags[k].id+'"  class=" col-1  category_item tag register-holder tags-holder  nav-item mb-3 me-3 me-lg-6 p-0" role="presentation"><div class="  nav-link d-flex justify-content-between flex-column flex-center overflow-hidden  h-100px py-4 px-0 active " data-bs-toggle="pill"  aria-selected="true" role="tab"><div class="nav-icon"><i class="ion-ios-pricetag-outline text-danger " style="font-size:30px"></i> </div><span class="nav-text text-gray-700 fw-bold fs-6 lh-1"><p>' + json.tags[k].name + '</p></span><span class="bullet-custom position-absolute bottom-0 w-100 h-4px bg-primary"></span></div></li>';
		...
		function processSuppliersResult(json) {
			...
			for (var k = 0; k < json.suppliers.length; k++) {
				...
				var supplier_item = $('<li data-supplier_id="' + json.suppliers[k].id + '"  class="  col-1  category_item supplier register-holder tags-holder  nav-item mb-3 me-1 me-lg-1 p-0" role="presentation">' +
					'<div class="nav-link d-flex justify-content-between flex-column flex-center overflow-hidden  h-100px px-0 active" data-bs-toggle="pill" aria-selected="true" role="tab">' +
						'<div class="nav-icon"><i class="fa fa-user" style="font-size:30px"></i></div>' +
		...
		var	back_to_categories_button = '<li id="back_to_categories" class=" col-1 nav-item mb-3 me-3 me-lg-6 h-100px" role="presentation"><a class="  nav-link d-flex justify-content-between flex-column flex-center overflow-hidden  h-100px py-6 active" data-bs-toggle="pill" href="#kt_stats_widget_2_tab_1" aria-selected="true" role="tab"><div class="nav-icon"><img class="rounded-3 mb-4" alt="" src="' + SITE_URL + '/assets/css_good/media/icons/icons8-back-50.png" class=""></div><span class="nav-text text-gray-700 fw-bold fs-6 lh-1" style="white-space:nowrap"></span><span class="bullet-custom position-absolute bottom-0 w-100 h-4px bg-primary"></span></a></li>';
			...
			for (var k = 0; k < json.categories_and_items.length; k++) {
				if (json.categories_and_items[k].type == 'category') {
					...
					var item = '<li data-has-variations="'+has_variations+'" data-id="'+json.categories_and_items[k].id+'" class=" col-1 category_item item   ' + image_class + '  ' + item_parent_class + '  nav-item mb-3  h-100px" role="presentation"><a class="  nav-link d-flex justify-content-between flex-column flex-center overflow-hidden h-100px py-4 active" data-bs-toggle="pill" href="#kt_stats_widget_2_tab_1" aria-selected="true" role="tab"><div class="nav-icon"> '+ prod_image +'</div><span class="nav-text text-gray-700 fw-bold fs-8 lh-1"><p>' + json.categories_and_items[k].name + '  <span class="text-bold">' + (json.categories_and_items[k].price ? '(' + decodeHtml(json.categories_and_items[k].price) + ')' : '') + '</span></p>  </span><span class="bullet-custom position-absolute bottom-0 w-100 h-4px bg-primary"></span></a></li>';
					$("#category_item_selection").append(item);
		...
		function processTagItemsResult(json) {
			...
			for (var k = 0; k < json.items.length; k++) {
				...
				var item = '<li data-has-variations="'+has_variations+'"  data-data-id ="'+json.items[k].id+'" class=" col-1 category_item item nav-item mb-3 me-3 me-lg-6 h-100px m-0" role="presentation"><a class="  nav-link d-flex justify-content-between flex-column flex-center overflow-hidden h-100px py-4 active p-1" data-bs-toggle="pill" href="#kt_stats_widget_2_tab_1" aria-selected="true" role="tab"><div class="nav-icon"><img class="rounded-3 mb-4 " alt="" src="' + image_src + '" class=""></div><span class="nav-text text-gray-700 fw-bold fs-7 lh-1"><p>' + json.items[k].name + '<br /> <span class="text-bold">' + (json.items[k].price ? '(' + json.items[k].price + ')' : '') + '</span></p></span><span class="bullet-custom position-absolute bottom-0 w-100 h-4px bg-primary"></span></a></li>';
