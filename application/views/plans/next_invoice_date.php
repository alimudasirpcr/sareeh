...
if (!empty($next_date)) {
    ...
    echo '<a class="text-center" href="' . erp_server_url(perfex_saas_get_invoice_payment_endpoint($invoice)) . '">';
    echo '<div class="mbot10"><span class="label label-' . ($next_days_left < 5 ? 'warning' : 'success') . ' tw-ml-3" data-toggle="tooltip" data-title="' . lang('perfex_saas_view_subscription_invoice') . '"><i class="fa-regular fa-eye fa-fw tw-mr-1"></i> ' . lang('next_invoice_date'). ': ' . e(date_as_display_date($next_date)) . '</b>' . '</span></div>';
    echo '</a>';
}
?>