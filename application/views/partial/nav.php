		...
		<ul class="nav nav-custom nav-tabs nav-line-tabs nav-line-tabs-2x border-0 fs-4 fw-semibold mb-n2 nav-wizard- <?php echo $progression ? '' : ''; ?>">
			<?php if($this->uri->segment(1) == 'items') { ?>
				...
				<?php if ($this->Employee->has_module_action_permission('items','edit_prices', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
					<li class="nav-item" ><?php echo anchor("items/pricing/".($item_info->item_id ? $item_info->item_id : -1).($query ? '?'.$query : ''),''.lang('edit_pricing'),array('class'=> ($this->uri->segment(2) == 'pricing') ? 'nav-link text-active-primary sp_link pb-4 active' : ' nav-link text-active-primary sp_link pb-4', 'role'=>'button', 'title'=>lang('edit_pricing')));?></li>
	 			<?php } ?>
