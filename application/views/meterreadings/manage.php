...
{	 
    ...
    enable_search('<?php echo site_url("$controller_name");?>',<?php echo json_encode(lang("confirm_search"));?>);
		...
		<?php } ?>
			enable_cleanup(<?php echo json_encode(lang("meterreadings_confirm_cleanup"));?>);
	...
	$("#generate_barcodes_form").submit(function()
	{
		...
		var url = '<?php echo site_url("meterreadings/generate_barcodes");?>'+'/'+selected.join('~')+'/'+num_labels_skip;
		window.location = url;
		return false;
	});
	...
	$('#generate_barcode_labels').click(function()
    {
    	...
    	$(this).attr('href','<?php echo site_url("meterreadings/generate_barcode_labels");?>/'+selected.join('~'));
    });
...
<div class="modal fade skip-labels" id="skip-labels" role="dialog" aria-labelledby="skipLabels" aria-hidden="true">
	...
	<div class="manage-row-options hidden">
		<div class="email_buttons meterreadings">
			<?php if(!$deleted) { ?>
						...
						<?php if ($this->Employee->has_module_action_permission($controller_name, 'delete', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
							<li>
								<?php echo 
									...
									'<span class="ion-loop"> '.lang("meterreadings_cleanup").'</span>',
									array('id'=>'cleanup', 'class'=>'','title'=>lang("meterreadings_cleanup"))); 
								?>
							</li>
						<?php }?>
