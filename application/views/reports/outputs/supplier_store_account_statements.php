			...
			<div class="panel-body">				
				...
				<?php foreach($report_data as $data) {?>
					...
					<div class="store_account_address">
						<span><strong><?php echo $supplier_title.' '.($data['supplier_info']->account_number ? $data['supplier_info']->account_number : '') ;?></strong></span><br />
						<?php if($data['supplier_info']->address_1) { ?>
					...
					<table class="table table-striped table-hover data-table tablesorter" id="sortable_table">
						<thead>
							<tr>
								...
								<td><?php echo lang('reports_receiving_id');?></td>
						...
						<tbody>
							...
							<tr>
								...
								<td><?php echo $transaction['receiving_id'] ? anchor('receivings/receipt/'.$transaction['receiving_id'], $this->config->item('receiving_prefix').' '.$transaction['receiving_id'], array('target' => '_blank')) : '-';?></td>
				...
				<?php if ($data['supplier_info']->email) {?>
				
					<div class="row">
						<div class="col-md-3 col-xs-12 col-sm-6 ">
						<?php 
						$email_url = str_replace('reports/generate/supplier_store_account_statements','reports/store_account_statements_email_supplier',current_url());
						$email_url = str_replace('supplier_id='.$this->input->get('supplier_id'),'supplier_id='.$data['supplier_info']->person_id,$email_url);
