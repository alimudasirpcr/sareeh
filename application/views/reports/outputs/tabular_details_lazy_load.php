...
<style>
	.row-container {
		display: flex;
		/* Display columns in a row */
		flex-wrap: wrap;
		/* Ensure columns wrap as needed */
	...
	.cell-list.column {
		...
		flex: 1;
		/* Ensures the columns take up equal space */
		min-width: 0;
		/* Prevents flex items from not shrinking */
	}

	.cell-list.column li {
		padding: 4px;
		border-bottom: 1px solid #eee;
		/* Visual separator for items */
	}
	...
	.clear-fix {
		clear: both;
		/* Ensures that the next row starts on a new line */
	}
</style>
		...
		<?php foreach ($overall_summary_data as $name => $value) { ?>
			<div class="col-md-3 col-xs-12 col-sm-6 ">
				<div class="info-seven primarybg-info position-relative me-5">
					...
					if ($name == 'items_having_warranty' || $name == 'items_having_no_warranty' || $name == 'number_items_counted' || $name == 'points_used' || $name == 'points_gained') {
				...
				<form id="config_columns" class="report-config hidden-print">
					<div class="piluku-dropdown btn-group table_buttons pull-right m-left-20">
						...
						<ul id="" class="dropdown-menu dropdown-menu-left col-config-dropdown" role="menu">
							...
							<?php $i = 0;
							foreach ($headersshow as $col_key) {
								$checked = '';
								if ($col_key['view'] == 1) {
							...
							?>
								<li class="col<?php echo $i; ?>">
									<span class="form-check form-check-sm form-check-custom form-check-solid"><input <?php echo $checked; ?> name="selected_columns[]" type="checkbox" class="columns form-check-input" id="<?php echo $col_key['column_id']; ?>" value="<?php echo $col_key['column_id']; ?>"><label class="sortable_column_name form-check-label " for="<?php echo $col_key['column_id']; ?>"><span></span><?php echo H($col_key['data']); ?></label><span class=""></span>
							</span>
								</li>
							<?php } ?>
						</ul>
