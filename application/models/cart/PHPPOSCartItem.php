...
abstract class PHPPOSCartItem extends PHPPOSCartItemBase
{
			...
			if ($data = parse_item_scan_data($this->scan))
			{
				...
				foreach($quantity_units as $qu)
				{
					...
					if(!$this->quantity_unit_id){
						if($this->type == 'sale'){
							if(isset($qu->default_for_sale) && $qu->default_for_sale == 1){
								$this->quantity_unit_id = $qu->id;
							}
						}else if($this->type == 'receiving'){
							if(isset($qu->default_for_recv) && $qu->default_for_recv == 1){
								$this->quantity_unit_id = $qu->id;
							}
						}
					}
	...
	private function load_item_defaults(array $params)
	{
		...
		{
			//Only write to properties that exist and not already set (added cart_line_supplier_id to avoid default supplier_id)
