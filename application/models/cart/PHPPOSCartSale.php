...
{		
	...
	public $customer_subscription_id;
	...
	public function return_order($receipt_sale_id)
	{
		...
		if ($CI->Delivery->has_delivery_for_sale($sale_id))
		{
			...
			if(isset($delivery_info['contact_preference'])){
				$delivery_info['contact_preference'] = is_serialized($delivery_info['contact_preference']) ? unserialize($delivery_info['contact_preference']) : $delivery_info['contact_preference'];
			}else{
	...
	public static function get_instance_from_sale_id($sale_id, $cart_id = NULL, $is_editing_previous = FALSE, $copy_price_rules = TRUE){
		...
		foreach($CI->Sale->get_sale_item_kits($sale_id)->result() as $row)
		{
			...
			$item_kit_props['receipt_line_sort_order'] = $row->receipt_line_sort_order;
		...
		if ($CI->Delivery->has_delivery_for_sale($sale_id))
		{
			...
			$cart->set_delivery_info($delivery_info);
	...
	function setup_defaults()
	{
		...
		{
			$this->create_work_order = 1;
		} else {
			$this->create_work_order = 0;
		}
				
		if($CI->config->item('create_invoices_for_customer_store_account_charges'))
		{
			$this->create_invoice = 1;
		}
		

		$this->show_terms_and_conditions = '';
		$this->customer_subscription_id = NULL;
...
// dd($this->get_override_tax_info());
	...
	*/
	...
	{
		if(isset($delivery_info['contact_reference'])){
			if(is_serialized($delivery_info['contact_reference'])){
				$delivery_info['contact_reference'] = unserialize($delivery_info['contact_reference']);
			}
		}
	...
	function update_register_cart_data()
	{
		...
		if($customer_id)
		{
			...
			$data['avatar']=$info->image_id ?  cacheable_app_file_url($info->image_id) : base_url()."assets/img/user.png"; //can be changed to  base_url()."img/avatar.png" if it is required
		}
		else
		{
			$data['customer']= NULL;
	...
	function process_barcode_scan($barcode_scan_data,$options = array())
	{
		...
		elseif($this->is_valid_item($barcode_scan_data))
		{
			...
			$item_to_add = new PHPPOSCartItemSale(array(
				'cart' => $this,
			...
			if(empty($variation) && !$secondary_supplier_id && $CI->Item->get_secondary_suppliers($item_to_add->item_id)->num_rows() > 0 && !$default_supplier_id && !$CI->config->item('disable_supplier_selection_on_sales_interface')){
			...
			if(empty($variation) && !$secondary_supplier_id && $CI->Item->get_secondary_suppliers($item_to_add->item_id)->num_rows() > 0 && !$default_supplier_id && !$CI->config->item('disable_supplier_selection_on_sales_interface')){
				...
				$this->validate_and_add_cart_item($item_to_add,$options, $secondary_supplier_id);
			}
			...
			**/
			...
			if (!empty($variation)) {
			    ...
			    $is_service = $item_to_add->is_service;
			    /*
				...
				*/
				...
				$CI->session->unset_userdata('is_service');
				$CI->session->set_userdata('is_service',$is_service);
	...
	public function validate_and_add_cart_item(PHPPOSCartItemBase $item_to_add,$options = array(), $secondary_supplier_id=null)
	{
		$CI =& get_instance();
	  ...
	  if ($item_to_add->below_cost_price())
	  {
		...
		{
			return $this->do_validate_and_add_cart_item($item_to_add,$options,$secondary_supplier_id);
		}
	...
	private function do_validate_and_add_cart_item(PHPPOSCartItemSale $item_to_add,$options = array(), $secondary_supplier_id=null)
	{
		$CI =& get_instance();
		...
		if (!$item_to_add->validate())
		{
			...
			if($CI->config->item('branding')['code'] == 'phpsalesmanager'){
				...
				if($CI->config->item('p4_api_bearer_token') && $CI->config->item('enable_p4_integration')){
		...
		if($secondary_supplier_id && !$item_to_add->variation_id){
			...
			$item_to_add->unit_price = $secondary_supplier->unit_price;
			$item_to_add->cost_price = $secondary_supplier->cost_price;
