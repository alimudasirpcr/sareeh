...
abstract class PHPPOSCartItemBase
{
	...
	public $line_index = NULL; //This is used to save item's temp index.
	public $receipt_line_sort_order = NULL;
	public $name;
	public $quantity = 1;
	public $unit_price;
	public $cost_price;
	...
	public $secondary_supplier_id;
	public $supplier_id;
	public $cart_line_supplier_id;
	
	public $main_image_id;
	
	public $weight;
		//Uses internally as a placeholder to calculate tax when there is a tax cap. Should only be set in Sale->save method
		public $calculated_sale_item_tax;
	...
	{		 
      ...
      throw new Exception("Cart Item type is set to a valid value (sale or receiving)");
		...
		if($this->receipt_line_sort_order == NULL){
			if(	isset($params['cart']) && get_class($params['cart']) == 'PHPPOSCartSale' 
				|| isset($params['cart']) && get_class($params['cart']) == 'PHPPOSCartRecv'){
				$params['cart']->make_clean_order();
				$this->receipt_line_sort_order = count($params['cart']->get_items()) + 1;
			}else{
				$this->receipt_line_sort_order = 0;
			}
		}
    ...
    $CI =& get_instance();
		...
		else
		{
			...
			{
				return $this->get_taxes_for_cart_item_not_saved_yet_tax_cap($cumulative_percent,$taxable_subtotal);
			}
			else
			{
				return $this->get_taxes_for_cart_item_not_saved_yet($cumulative_percent);
			}
		}	
	}
	...
	public function get_taxes_for_cart_item_not_saved_yet($cumulative_percent = 0)
	{
    $CI =& get_instance();
...
public function get_taxes_for_cart_item_not_saved_yet_offline($cumulative_percent = 0)
	...
	{
		return to_currency_no_money(($this->get_modifiers_subtotal() - ($this->get_modifiers_subtotal() * $this->discount / 100))+ ($this->unit_price*$this->quantity-$this->unit_price*$this->quantity*$this->discount/100));
	}	
