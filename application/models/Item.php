...
class Item extends MY_Model
{
	...
	*/
	...
	function get_all_offline($limit=100,$offset=0 , $hide_out_of_stock_grid = FALSE, $show_hidden=FALSE)
	{				
					save_query();
...
return $result;
	...
	*/
	function get_all($deleted=0,$limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		$see_all_items = $this->Employee->has_module_action_permission('items','see_all_items',$this->Employee->get_logged_in_employee_info()->person_id);
		...
		if ($attribute_count > 0 && !$this->config->item('dont_show_images_in_search_suggestions'))
		{
	        ...
	        LEFT JOIN `$phppos_location_item_variations` 
	          ON `$phppos_location_item_variations`.`item_variation_id` = `$phppos_item_variations`.`id` and `$phppos_location_item_variations`.`location_id` = $current_location
					...
					(!$see_all_items?
					" LEFT JOIN `$phppos_location_ban_items`
						ON (`$phppos_location_ban_items`.`item_id` = $phppos_items.item_id and `$phppos_location_ban_items`.`location_id` = $location_id)
					WHERE `$phppos_location_ban_items`.`item_id` is NULL ":" ").
		...
		else
		{
			...
			if(!$see_all_items)
				$this->db->join('location_ban_items', 'location_ban_items.item_id = items.item_id and location_ban_items.location_id = '.$location_id, 'left');
			...
			if(!$see_all_items)
				$this->db->where('location_ban_items.location_id Is NULL');
			if (!$this->config->item('speed_up_search_queries'))
			{
	...
	function get_all_by_category($category_id, $hide_out_of_stock_grid = FALSE, $offset=0, $limit = 14,$show_hidden=FALSE)
	{
		...
		else
		{
			...
			(SELECT CONCAT('KIT ',item_kit_id), unit_price, name, '', 'no_image' as image_id  , $item_kits_table.tax_included , $item_kits_table.override_default_tax
			FROM $item_kits_table 
	...
	function get_all_favorite_items($hide_out_of_stock_grid = FALSE, $offset=0, $limit = 14)
	{
		...
		if (!$hide_out_of_stock_grid)
		{
			...
			UNION ALL (
			SELECT CONCAT('KIT ',item_kit_id), unit_price, name, main_image_id as image_id,'' as SIZE , $item_kits_table.tax_included , $item_kits_table.override_default_tax
			FROM $item_kits_table 
			WHERE deleted = 0 
			and $item_kits_table.is_favorite = 1
			and $item_kits_table.item_kit_id NOT IN (SELECT item_kit_id FROM phppos_grid_hidden_item_kits WHERE location_id=$location_id) 
		...
		else
		{
			$location_items_table = $this->db->dbprefix('location_items ');
			$current_location=$this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
			$result = $this->db->query("(SELECT i.item_id, i.unit_price, name,size, image_id , i.tax_included, 
			i.override_default_tax, FROM $items_table as i LEFT JOIN $items_images_table USING (item_id) 
			LEFT JOIN $location_items_table as li ON i.item_id = li.item_id and li.location_id = $current_location
			WHERE (quantity > 0 or quantity IS NULL or is_service = 1) 
			and deleted = 0 
			and system_item = 0 
	...
	function get_all_item_by_supplier($supplier_id, $hide_out_of_stock_grid = FALSE, $offset=0, $limit = 14)
	{
		...
		{
			$result = $this->db->query("
			SELECT item_id, unit_price,name, image_id, size ,tax_included, $items_table.override_default_tax
			FROM $items_table 
			LEFT JOIN $items_images_table USING (item_id) 
			WHERE deleted = 0 
			and system_item = 0 
	...
	function count_all($deleted=0)
	{
		...
		$this->db->from('items');
		if(!$see_all_items){
			$this->db->join('location_ban_items', 'items.item_id = location_ban_items.item_id and location_ban_items.location_id = '.$location_id,'left');
			$this->db->where('location_ban_items.location_id IS NULL');
		}
	...
	*/
	...
	{
		$item_id = str_replace('|FORCE_ITEM_ID|','',$item_id ? $item_id : '');
		...
		else
		{
			...
			//Get all the fields from items table
			$fields = array('weight_unit','ecommerce_inventory_item_id','barcode_name','info_popup','name','category_id','supplier_id','manufacturer_id','item_number','product_id','ecommerce_product_id','ecommerce_product_quantity','description','size','tax_included','cost_price','unit_price','promo_price','start_date','end_date','reorder_level','expire_days','item_id','allow_alt_description','is_serialized','override_default_tax','is_ecommerce','is_service','is_ebt_item','commission_percent','commission_percent_type','commission_fixed','change_cost_price','disable_loyalty','deleted','last_modified','ecommerce_last_modified','tax_class_id','replenish_level','system_item','max_discount_percent','max_edit_price','min_edit_price','custom_field_1_value','custom_field_2_value','custom_field_3_value','custom_field_4_value','custom_field_5_value','custom_field_6_value','custom_field_7_value','custom_field_8_value','custom_field_9_value','custom_field_10_value','required_age','verify_age','weight','length','width','height','ecommerce_shipping_class_id','long_description','allow_price_override_regardless_of_permissions','main_image_id','only_integer','is_series_package','series_quantity','series_days_to_use_within','is_barcoded','item_inactive','default_quantity','is_favorite','loyalty_multiplier' , 'assigned_repair_item', 'assigned_to' , 'approved_by');
	...
	public function lookup_item_by_item_id($item_id)
	{
		$item_id = str_replace('|FORCE_ITEM_ID|','',$item_id);
	...
	//return item_id
	public function lookup_item_by_item_number($item_number)
	{
		$this->db->from('items');
		$this->db->where('item_number',$item_number);
	...
	public function lookup_item_by_product_id($product_id)
	{
		$this->db->from('items');
		$this->db->where('product_id', $product_id); 
	...
	public function lookup_item_by_item_name($item_name)
	{
		$this->db->from('items');
	...
	*/
	function save(&$item_data, $item_id = false)
	{
		...
		if (!$item_id or !$this->exists($item_id))
		{
			if($this->db->insert('items',$item_data))
			{
				...
				if ($this->config->item('new_item_web_hook'))
				{
		...
		if($this->config->item('edit_item_web_hook'))
		{
			$item_data['item_id'] = $item_id;
			do_webhook($item_data, $this->config->item('edit_item_web_hook'));
		}
		return $return;
	}
	...
	function get_custom_field($number,$key="name")
	{
		...
		if (!$config_data)
		{
			$config_data = $this->config->item('item_custom_field_prefs') ? unserialize($this->config->item('item_custom_field_prefs')) : array();
		}
  ...
  */
  function add_variation_data(&$suggestions, $search, $price_field)
  {
	...
	*/
	function get_item_search_suggestions_without_variations($search,$deleted = 0,$limit=25,$price_field = FALSE,$hide_inactive = false, $supplier_id = false)
	{
		if (!trim($search))
		{
			return array();
		...
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++)
		{
			if ($this->get_custom_field($k)) 
			{
				...
				if($supplier_id){
					...
					$this->db->group_start();
                    $this->db->where('items.supplier_id', $supplier_id);
					$this->db->or_where('items_secondary_suppliers.supplier_id', $supplier_id);
                    $this->db->group_end();
					$this->db->group_by('items.item_id, items.category_id');
				}
			  ...
			  //Try employee
				...
				'can_override_price_adjustments' => $can_override_price_adjustments,
						...
						'label' => $row->custom_field.($price_field ? ' - '.to_currency($row->$price_field) : ''),
						'secondary_suppliers' => $this->get_secondary_suppliers($row->item_id)->result_array(),
						'default_supplier' => $this->get_default_supplier($row->item_id)->result_array()
					);
	...
	function get_item_search_suggestions($search,$deleted=0,$price_field = 'unit_price',$limit=25,$hide_inactive = false, $supplier_id = false)
	{
		...
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++)
		{
			if ($this->get_custom_field($k)) 
			{
				...
				if ($this->get_custom_field($k,'type') != 'date')
				{
					$custom_field = 'custom_field_'.$k.'_value';
				}
				else
				{
					$custom_field = 'FROM_UNIXTIME(custom_field_'.$k.'_value, "'.get_mysql_date_format().'")';
				}

				$this->db->select('location_items.quantity as quantity, '.$custom_field.', item_variations.deleted as variation_deleted,items.main_image_id as image_id, (CASE WHEN '.$item_variations_table.'.supplier_id THEN '.$item_variations_table.'.supplier_id ELSE '.$items_table.'.supplier_id END) AS supplier_id', false);
				$this->db->select('items.*, item_variations.id as item_variation_id, categories.name as category', false);				
				$this->db->from('items');
				...
				if($supplier_id){
					...
					$this->db->or_where('items_secondary_suppliers.supplier_id', $supplier_id);
                    $this->db->group_end();
					$this->db->group_by('items.item_id, items.category_id, item_variations.id, location_items.location_id');
				}
			  ...
			  //Try employee
				...
				//code for offline pos
					$data = array(
						...
						'secondary_suppliers' => $this->get_secondary_suppliers($row->item_id)->result_array(),
						'supplier_name' => $this->Supplier->get_name($row->supplier_id),
						'default_supplier' => $this->get_default_supplier($row->item_id)->result_array()
					);
		...
		if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
		{
		  // Query 1:
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,items.main_image_id as image_id, item_variations.id as item_variation_id, categories.name as category, MATCH (".$items_table.".name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$items_table.".name = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
			$this->db->from('items');
			$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			$this->db->join('item_variations', 'items.item_id = item_variations.item_id','left');
			...
			$this->db->select("$quantity_field as quantity,item_variations.item_number as item_variation_number,item_variations.deleted as variation_deleted,items.main_image_id as image_id,item_variations.id as item_variation_id, categories.name as category, MATCH (".$items_table.".item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$items_table.".item_number = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			...
			$this->db->group_start();
			$this->db->where("MATCH (".$items_table.".item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE)", NULL, FALSE);
			$this->db->or_like($item_variations_table.'.item_number', $search,!$this->config->item('speed_up_search_queries') ? 'both' : 'after');
			$this->db->group_end();
			$this->db->limit($limit);
			$this->db->order_by('rel DESC, exact_score DESC');
			$this->db->group_by('items.item_id, item_variations.id');
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,items.main_image_id as image_id, item_variations.id as item_variation_id, categories.name as category,MATCH (product_id) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$items_table.".product_id = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
		...
		else
		{
			...
			$this->db->select("$quantity_field as quantity, item_variations.deleted as variation_deleted, item_variations.id as item_variation_id, items.main_image_id as image_id, categories.name as category, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
			$this->db->from('items');
			...
			$this->db->select("$quantity_field as quantity, item_variations.item_number as item_variation_number, item_variations.deleted as variation_deleted, items.item_id, items.name, items.item_number, items.product_id, item_variations.id as item_variation_id, items.main_image_id as image_id,categories.name as category, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
			...
			$this->db->group_by('items.item_id, item_variations.id');
			$this->db->group_start(); 
			$this->db->like($items_table.'.item_number', $search,!$this->config->item('speed_up_search_queries') ? 'both' : 'after');
			$this->db->or_like($item_variations_table.'.item_number', $search,!$this->config->item('speed_up_search_queries') ? 'both' : 'after');
			$this->db->group_end(); 
			$this->db->limit($limit);
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,items.item_id, items.name, items.item_number, items.product_id, item_variations.id as item_variation_id, items.main_image_id as image_id, categories.name as category, (CASE WHEN ".$item_variations_table.".supplier_id THEN ".$item_variations_table.".supplier_id ELSE ".$items_table.".supplier_id END) AS supplier_id", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			...
			$this->db->where('items.system_item',0);
			$this->db->like($items_table.'.product_id', $search,!$this->config->item('speed_up_search_queries') ? 'both' : 'after');
			$this->db->group_by('items.item_id, item_variations.id');
			$this->db->limit($limit);
	...
	/*
		...
		$location_id = NULL: current employee location, 0: all location
	*/
	...
	function search($search, $deleted=0,$category_id = false, $limit=20, $offset=0, $column='name', $orderby='asc', $fields = 'all')
	{
		...
		$location_id= $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
		$are_any_items_banned = $this->are_any_items_banned($location_id);
		if($see_all_items){
			$location_ban_item_list_query_left_join = "";
			$location_ban_item_list_query_where = "";
		}else{
			$location_ban_item_list_query_left_join = " LEFT JOIN `$phppos_location_ban_items` ON (`$phppos_items`.`item_id` = $phppos_location_ban_items.`item_id` and `$phppos_location_ban_items`.`location_id`=$location_id ) ";
			$location_ban_item_list_query_where = " and `$phppos_location_ban_items`.`location_id` is NULL ";
		}

		if (!$this->config->item('speed_up_search_queries'))
		{	
			$quantity_field ="IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0, 
				SUM(IF($phppos_item_variations.deleted=0, $phppos_location_item_variations.quantity, 0)) ".
				(($search && $fields == 'all') ?  "/ GREATEST(count(DISTINCT $phppos_additional_item_numbers.item_number), 1) " : "") .
				",$phppos_location_items.quantity)";
		}
		else
		{
	        ...
	        LEFT JOIN `$phppos_location_item_variations`
	          ON `$phppos_location_item_variations`.`item_variation_id` = `$phppos_item_variations`.`id` and `$phppos_location_item_variations`.`location_id` = $current_location
					...
					LEFT JOIN `$phppos_categories`
						ON `$phppos_categories`.`id` = $phppos_items.category_id
					$location_ban_item_list_query_left_join
					$search_main_query $location_ban_item_list_query_where
		...
		else
		{
			...
			if(!$see_all_items){
				if($are_any_items_banned)
				{
					$this->db->where($this->db->dbprefix('location_ban_items').'.item_id', NULL);
				}
			}
	...
	function search_count_all($search, $deleted = 0,$category_id = FALSE, $limit=10000, $fields = 'all')
	{
		...
		if(!$see_all_items){
			$this->db->join('location_ban_items', 'items.item_id = location_ban_items.item_id and location_ban_items.location_id = '.$current_location, 'left');
			$this->db->where('location_ban_items.location_id IS NULL');
		}
	...
	function create_or_update_item_by_name($item_name,$can_cache = TRUE)
	{
		$item_id = FALSE;
	...
	/*
	Gets sale price for item given an array of parameters. Current keys are item_id, variation_id, quantity_unit_id,quantity_unit_quantity and tier_id
	*/
	function get_sale_price(array $params)
	{
		...
		if ($item_info->is_recurring && !isset($params['ignore_recurring_price']))
		{
			$startup_cost = $item_info->startup_cost;
			
			if ($item_info->prorated)
			{
				$params['ignore_recurring_price'] = TRUE;
				$normal_price = $this->get_sale_price($params);				
				
				if ($item_info->interval == 'weekly')
				{
					$normal_bill_cycle = 7;
				}
				elseif($item_info->interval == 'monthly_on_day_of_month' || $item_info->interval == 'monthly_on_day_of_week')
				{
					$normal_bill_cycle = 30;
				}
				elseif($item_info->interval == 'yearly_on_date' || $item_info->interval == 'yearly_on_month_on_day_of_week')
				{
					$normal_bill_cycle = 365;
				}
				
				$this->load->model('Customer_subscription');
				$next_payment_date = $this->Customer_subscription->get_next_payment_date($item_info);
				$this->load->helper('date');
				$days_unit_next_payment_date = days_between_dates($next_payment_date,date('Y-m-d'));
				
				$prorated_amount = $normal_price / ($normal_bill_cycle/$days_unit_next_payment_date);
				$startup_cost+=$prorated_amount;
			}
			
			return to_currency_no_money($startup_cost);
		}
		if ($quantity_unit_id)
		{
		...
		elseif(($variation_id && $variation_info->unit_price) || ($variation_id && $variation_location_info && $variation_location_info->unit_price) || ($variation_id && $variation_info->promo_price) || ($variation_id && $variation_location_info && property_exists($variation_location_info, 'promo_price')))
		{
	...
	function add_ban_location($item_id,$location_id = false)
	{
		if (!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
		...
		return $this->db->replace('location_ban_items',array('item_id' => $item_id,'location_id' => $location_id));
	}
	
	function remove_ban_location($item_id,$location_id = false)
	{
		if (!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
		...
		return $this->db->delete('location_ban_items');
	}

	
	function are_any_items_banned($location_id = NULL)
	{
		if (!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
		}
		$this->db->from('location_ban_items');
		$this->db->where('location_id',$location_id);
	...
	function get_banned_items($item_ids,$location_id = NULL)
	{
		...
		$this->db->from('location_ban_tags');
		$this->db->join('tags','tags.id=location_ban_tags.tag_id');
		$this->db->join('items_tags','tags.id=items_tags.tag_id');
		$this->db->where('location_id',$location_id);
		if (!empty($item_ids))
		{
			$this->db->group_start();
			...
			foreach($item_ids_chunk as $item_ids)
			{
				$this->db->or_where_in('items_tags.item_id',$item_ids);
			}
			$this->db->group_end();
		}
		else
	...
	function is_item_ban($item_id,$location_id = false)
	{
		if (!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
	...
	function get_secondary_suppliers($item_id)
	{
		$this->db->select('items_secondary_suppliers.*, suppliers.company_name, people.full_name');
		$this->db->from('items_secondary_suppliers');
		$this->db->join('suppliers', 'suppliers.person_id = items_secondary_suppliers.supplier_id');
		$this->db->join('people', 'suppliers.person_id = people.person_id');
		$this->db->where('item_id',$item_id);
		return $this->db->get();
	}

	function get_default_supplier($item_id)
	{
		$this->db->select('items.item_id, items.supplier_id, items.cost_price, items.unit_price, suppliers.company_name, people.full_name');
		$this->db->from('items');
	...
	function save_secondary_supplier($item_id,$supplier_id,$sec_supplier_id = NULL)
	{
		if ($sec_supplier_id > 0)
		{
			$this->db->where('id',$sec_supplier_id);
			$this->db->update('items_secondary_suppliers',array('item_id' => $item_id,'supplier_id' => $supplier_id));
		}
		else
		{
			$this->db->replace('items_secondary_suppliers',array('item_id' => $item_id,'supplier_id' => $supplier_id));
		}
	}
	
	function delete_secondary_supplier($sec_supplier_id)
	{
		$this->db->where('id',$sec_supplier_id);
		$this->db->delete('items_secondary_suppliers');
	}

	function update_secondary_supplier_price($cost_price, $unit_price, $sec_supplier_id)
	{
		$this->db->where('id',$sec_supplier_id);
		$this->db->update('items_secondary_suppliers',array('cost_price' => $cost_price, 'unit_price' => $unit_price));
	}

	function get_secondary_supplier_details($item_id, $supplier_id)
	{
		$this->db->from('items_secondary_suppliers');
		$this->db->where(array('item_id' => $item_id, 'supplier_id' => $supplier_id));
		return $this->db->get()->row();
	}

	function get_all_suppliers_of_an_item($item_id){
		$this->db->save_queries = TRUE;
		$this->db->select('items_secondary_suppliers.supplier_id, items_secondary_suppliers.cost_price,  items_secondary_suppliers.unit_price,  suppliers.company_name, people.full_name');
		$this->db->from('items_secondary_suppliers');
		$this->db->join('suppliers', 'suppliers.person_id = items_secondary_suppliers.supplier_id');
		$this->db->join('people', 'suppliers.person_id = people.person_id');
		$this->db->where('item_id',$item_id);
		
		$sup_query1 = $this->db->get_compiled_select();

		$this->db->select('items.supplier_id, items.cost_price,  items.unit_price,  suppliers.company_name, people.full_name');
		$this->db->from('items');
		$this->db->join('suppliers', 'suppliers.person_id = items.supplier_id');
		$this->db->join('people', 'suppliers.person_id = people.person_id');
		$this->db->where('item_id',$item_id);
	...
	function get_location_ban_item_list($location_id, $can_cache = TRUE){
		if($location_id == 0) return array();
		$ban_array = array();
		if($can_cache){
			static $cache = array();
		}else{
		...
		foreach($this->db->get()->result_array() as $row)
		{
			array_push($ban_array,$row['item_id']);
		}
		$cache[$location_id] = $ban_array;
		return $ban_array;
	}

	function filter_location_item($result_array, $location_id = false, $can_cache = TRUE){

		if(count($result_array) == 0) return array();

		if($can_cache){
			static $see_all_items_1 = -1;
			static $location_id_1 = -1;

			if($see_all_items_1 === -1){
				$see_all_items_1 = $this->Employee->has_module_action_permission('items','see_all_items',$this->Employee->get_logged_in_employee_info()->person_id);
			}
			if($location_id_1 === -1){
				$location_id_1 = $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
			}
			$see_all_items = $see_all_items_1;
			if (!$location_id)
			{
		...
		if($see_all_items) return $result_array;
		$item_ids_in_search_result = array_column($result_array,'value');
		$item_ids_banned_in_search_result = $this->get_banned_items($item_ids_in_search_result,$location_id);

		$filter_array = array();
		for($iResult = 0; $iResult< count($result_array); $iResult ++){
			$eResult = $result_array[$iResult];
			if(!in_array($eResult['value'], $item_ids_banned_in_search_result)){
				array_push($filter_array, $eResult);
			}
		}
		return $filter_array;
	}

	function has_variations($item_id){
		$this->load->model('Item_variations');

		if($this->Item_variations->get_all($item_id, true)->num_rows() >= 1){
			return true;
		}
		return false;
	}

	function has_secondary_supplier($item_id){
		if($this->get_secondary_suppliers($item_id)->num_rows() >= 1){
			return true;
		}
		return false;
	}

	function get_variations_for_item_based_on_supplier($item_id, $supplier_id){
		$this->load->model('Item_variations');
		return $this->db->get_where('item_variations', array('item_id' => $item_id, 'supplier_id' => $supplier_id))->result();
	}

	function get_new_variation_based_on_selected_supplier($attribute_group, $variation_group){
	
		$this->db->select('attribute_value_id, item_variation_id, GROUP_CONCAT(attribute_value_id ORDER BY attribute_value_id) AS attribute_group', true);
		$this->db->from("item_variation_attribute_values");
		$this->db->where_in("attribute_value_id", $attribute_group);
		$this->db->where_in("item_variation_id", $variation_group);
		$this->db->group_by('item_variation_id');

		$sup_query = $this->db->get_compiled_select();

		$sql = "select * from ($sup_query) tmp where tmp.attribute_group = '".implode(",",$attribute_group)."'";
		...
		if($query->num_rows() >= 1){
			$variation_id = $query->row('item_variation_id');
		}
	...
	function get_work_order_item_id()
	{
		$item_id = FALSE;
	...
	function get_serialized_work_order_item_id()
	{
		$item_id = FALSE;
	...
	function check_exist_instock_serialnumber($item_id,$serial_number,$in_stock=1)
	{
	...
	function merge($items,$item_to_merge)
	{

		// Get employee and location information
		$employee_id = $this->Employee->get_logged_in_employee_info() && $this->Employee->get_logged_in_employee_info()->person_id ? $this->Employee->get_logged_in_employee_info()->person_id : 1;
		$location_id= $this->Employee->get_logged_in_employee_current_location_id() ? $this->Employee->get_logged_in_employee_current_location_id() : 1;
		...
		{
			$item_info 		= 	$this->get_info($item_id);
			$item_quantity  =   $this->Item_location->get_info($item_id);
			$new_quantity	+=	$item_quantity->quantity ? $item_quantity->quantity : 0;
		}
		
		$merge_item_info	= 	$this->Item_location->get_info($item_to_merge);
		$new_quantity 		+=	$merge_item_info->quantity ? $merge_item_info->quantity : 0;			
		...
		if (count($items) > 0)
		{
			...
			// Update item_id in sales_items table for all sales that have items in $items 
			$this->db->where_in('item_id',$items);
			$this->db->update('sales_items',array('item_id' => $item_to_merge));

			// Update trans_items in inventory table for all inventory transactions that have items in $items
			$this->db->where_in('trans_items',$items);
			$this->db->update('inventory',array('trans_items' => $item_to_merge));
			
			if (empty($merge_item_info->quantity)) {
				$item_location_data['item_id'] 		= $item_to_merge;
				$item_location_data['location_id'] 	= $location_id;
				$item_location_data['quantity'] 	= $new_quantity;
				$this->db->insert('location_items',$item_location_data);
			} else {
				$this->db->where_in('item_id',$item_to_merge);
				$this->db->update('location_items',array('quantity' => $new_quantity));
			}
			
			$updateInventoryArr = array(
				'trans_comment' 			=>	'Item Merge with '.$item_info->name,
				'trans_items' 				=> 	$item_to_merge,
				'trans_inventory' 			=> 	$item_quantity->quantity,
				'trans_user' 				=> 	$employee_id,
				'location_id' 				=> 	$location_id,
				'trans_current_quantity' 	=>	$new_quantity,
			);
			$this->Inventory->insert($updateInventoryArr);
