...
class Sale extends MY_Model
{
	...
	function get_payment_sales_total_for_shift($shift_start, $shift_end,$payment_type)
  {
	...
	{	
		...
		if ($sale_id)
		{
			$delivery_info = $this->Delivery->get_info_from_sale_id($sale_id)->row_array();
			
			if (isset($delivery_info['id']) && $delivery_info['id'])
			{
				foreach($this->Delivery->get_files($delivery_info['id'])->result_array() as $file)
				{
					$delivery_file_ids[] = $file['file_id'];
				}
			}
		...
		{
			$sales_data["custom_field_${k}_value"] = $cart->{"custom_field_${k}_value"};
		}
		...
		if (((!$cart->is_ecommerce && $suspended < 2) || ($this->config->item('import_ecommerce_orders_suspended') && $suspended < 2)))
		{
		 ...
		 if ($sale_id !== FALSE && $before_save_sale_info && $before_save_sale_info->customer_id && $before_save_sale_info->suspended != 2)
		 {
			...
			if (property_exists($item,'item_id'))
			{
				...
				if ($item->is_recurring)
				{
					...
					array(
						...
						'customer_id' => $customer_id,
						'status' => 'current',
						'interval' => $cur_item_info->interval,
						'weekday' => $cur_item_info->weekday,
						'day_number' => $cur_item_info->day_number,
						'month' => $cur_item_info->month,
						'day' => $cur_item_info->day,
						'card_on_file_token' => $this->session->userdata('cc_token'),
						'card_on_file_masked' => $this->session->userdata('masked_account'),
						'startup_cost' => $item->unit_price,
						'recurring_charge_amount' => $this->Item->get_sale_price(array('ignore_recurring_price' => TRUE,'item_id' => $item->item_id,'variation_id' => $item->variation_id)),
						'location_id' => $cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id(),
					);
					
					$this->Customer_subscription->save($customer_subscription_data);
					
					$customer_subscription_data['next_payment_date'] = $this->Customer_subscription->get_next_payment_date($customer_subscription_data['id']);
					$this->Customer_subscription->save($customer_subscription_data,$customer_subscription_data['id']);
				...
				// if create_work_order is set, we need to create a work order for this item
				if($cart->create_work_order) 
				{
					$is_item_repair = 1;
				} else {
				...
				(
					...
					'line'=>$line,
					'receipt_line_sort_order'=>$item->receipt_line_sort_order,
					...
					'item_cost_price' =>  $global_weighted_average_cost === FALSE ? (float)to_currency_no_money($cost_price+ $item->get_modifier_cost_total(),10) : (float)$global_weighted_average_cost,
					'item_unit_price'=>(float)$item->unit_price + (float)$item->get_modifier_unit_total(),
					'regular_item_unit_price_at_time_of_sale' =>(float)$item->regular_price + (float)$item->get_modifier_unit_total(),
					'commission' => (float)$line_item_commission,
					'subtotal' => (float)$sale_item_subtotal,
					'total' => (float)$sale_item_total,
					'tax' => (float)$sale_item_tax,
					'profit' =>(float)$sale_item_profit,
					'tier_id' => $item->tier_id ? $item->tier_id : NULL,
					'damaged_qty' => $item->damaged_qty ? (float)$item->damaged_qty : 0.0000000000	,
					'override_taxes' =>  $sales_items_override_taxes? serialize($sales_items_override_taxes) : 0.0000000000	,
					'unit_quantity' => $item->quantity_unit_quantity !== NULL ? (float)$item->quantity_unit_quantity : 0.0000000000	,
		...
		if($cart->get_has_delivery() && $sale_total_qty >= 0)
		{
			...
			{
				if(!isset($delivery_info['location_id'])){
				...
				$delivery_info['tracking_number'] = $delivery_info['tracking_number'] ? $delivery_info['tracking_number'] : ($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS').''.$sale_id;
				...
				foreach($delivery_file_ids as $file_id)
				{
					$this->Delivery->add_file($delivery_info['id'],$file_id);
				}
			}
	    ...
	    //If we have saved a sale we need to confirm the payments in the cart
        $this->load->model('Credit_card_charge_unconfirmed');
        foreach($cart->get_credit_card_payments_charge_ids() as $payment_charge_id)
        {
            $this->Credit_card_charge_unconfirmed->delete($payment_charge_id);
        }

		// Create Work Order if create_work_order is set to true in the cart 
		...
		if ($cart->create_invoice && $balance)
		{
			...
			$this->Invoice->save('customer',$invoice_data,$this->Invoice->get_invoice_for_order_id('customer',$sale_id));
			if ($this->Invoice->get_invoice_for_order_id('customer',$sale_id))
			{
				$invoice_id = $this->Invoice->get_invoice_for_order_id('customer',$sale_id);
				$this->Invoice->delete_invoice_details_by_order('customer',$sale_id);
			}
			else
			{
			...
			if ($this->config->item('automatically_email_invoice'))
			{
				$this->email_invoice('customer', $invoice_id);
			}
			$details_data = array();
			$details_data['invoice_id'] = $invoice_id;
			$details_data['sale_id'] 	= $sale_id;
			$details_data['total'] 		= $balance;
			
			$this->Invoice->save_invoice_details('customer',$details_data);
		}
		...
		if($this->Location->get_info_for_key('sidekick_auto_review') && $cart->customer_id){
			//sidekick insert/update
			$location = $this->Location->get_info($this->Employee->get_logged_in_employee_current_location_id());
			$customer = $this->Customer->get_info($cart->customer_id);
			$this->load->helper('webhook');
			if(($customer->email || $customer->phone_number) && $location->sidekick_api_key){
				$customer_data = array(
				...
				do_webhook($customer_data, 'https://rest.gohighlevel.com/v1/contacts/', $headers);
			}
		}
	...
	function delete($sale_id, $all_data = false, $update_ecommerce=TRUE)
	{
		...
		if ($all_data)
		{
			...
			if (isset($delivery_info['id']) && $delivery_info['id'])
			{
				$this->db->delete('delivery_files', array('delivery_id' => $delivery_info['id'])); 
			}
	...
	function get_sale_item_modifiers($sale_id,$item_id,$line,$modifier_id=false)
	{
		...
		$this->db->where('line', $line);
		if($modifier_id){
			$this->db->where('modifier_item_id ', $modifier_id);
		}
		$this->db->order_by('line');
		return $this->db->get();
	}
	...
	function get_sale_item_kits($sale_id, $line = NULL, $is_repair = null)
	{
		$this->db->from('sales_item_kits');
		$this->db->where('sale_id',$sale_id);
		if(isset($line)) {
		...
		if(isset($is_repair)) {
			$this->db->where('is_repair_item',$is_repair);
		}
		$this->db->order_by('line');
		return $this->db->get();
	}

	function get_item_kits_sale($sale_id, $item_kit_id, $is_repair = 0)
	{
		$this->db->from('sales_item_kits');
		$this->db->where('sale_id',$sale_id);
		$this->db->where('item_kit_id',$item_kit_id);
	...
	function get_suspended_sales_displayable_columns( $is_quick = false )
	{
		...
		if ($this->uri->segment(2) == 'work_orders' && !$is_quick)
		{
			...
			$return['sale_type_name']['html'] = TRUE;
		}
	  for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
	  {
	...
	function get_quantity_sold_for_item_in_sale($sale_id, $item_id,$variation_id = NULL)
	{
		...
		foreach($this->db->get()->result_array() as $row)
		{
			$total+=$row['quantity_purchased']*($row['unit_quantity'] !== NULL ? $row['unit_quantity'] : 1);
		}
		return $total;
	}
	...
	function add_sale_item($sale_id,$item_id,$quantity=1,$serialnumber=null,$is_repair_item=false){
		...
		if($serialnumber){
			$item_id_temp = $this->Item_serial_number->get_item_id($serialnumber)!== FALSE ? $serialnumber : NULL;
			if ($item_id_temp){
				$item_id = $item_id_temp;
				$quantity = 1;
				$unit_price = $this->Item_serial_number->get_price_for_serial($serialnumber);
				$cost_price = $this->Item_serial_number->get_cost_price_for_serial($serialnumber);
			}
		}
		...
		// Verify if customer has a tier and if unit price is exists
		if($unit_price && $customer_info->tier_id){
			$tier_array = array(
				'item_id' 		=> $item_id,
				'tier_id' 		=> $customer_info->tier_id,
			);
			$unit_price = $this->Item->get_sale_price($tier_array);
		}
	...
	function add_sale_item_kit($sale_id,$item_kit_id,$quantity=1,$is_repair_item=true) {
		...
		$variation_id 	= NULL;
		$unit_price 	= 0;
		$item_kit_info 	= $this->Item_kit->get_info($item_kit_id,false);
		$quantity 		= 1;
		
		if(!$unit_price){
			$unit_price = $this->Item_kit->get_sale_price(array('item_kit_id' => $item_kit_id));
		}
		...
		$cur_item_location_info = $this->Item_kit_location->get_info($item_kit_id);
		$cur_item_variation_info = $this->Item_variations->get_info($variation_id);


		$cost_price 	= ($cur_item_location_info && $cur_item_location_info->cost_price) ? $cur_item_location_info->cost_price : $item_kit_info->cost_price;
		$regular_price 	= ($cur_item_location_info && $cur_item_location_info->unit_price) ? $cur_item_location_info->unit_price : $item_kit_info->unit_price;
		$subtotal 		= $quantity * $unit_price;
		$customer_id 	= $this->get_info($sale_id)->row()->customer_id;
		$customer_info 	= $this->Customer->get_info($customer_id);

		if(!$customer_id || $customer_info->taxable){
			$tax = $this->get_taxes_for_sale_item($item_kit_id,$unit_price,$quantity);
		} else{
			$tax = 0;
		}
		
		if(!$this->Item_kit->exists($item_kit_id)){
			return false;
		}
		...
		$sales_item_data = array(
			...
			'regular_item_kit_unit_price_at_time_of_sale'	=>	$regular_price,
			'quantity_purchased'		=>	$quantity,
			'subtotal'					=>	$subtotal,
			'tax'						=>	$tax,
			'total'						=>	$subtotal+$tax,
		...
		$repair_item_id = $this->work_order->create_or_update_repair_item_kits();

		if($item_kit_id == $repair_item_id){
			$sales_item_data['is_repair_item'] = 1;
		}
	...
	function sale_item_quantity_update($sale_id,$item_id,$quantity, $line = false){
	...
	function sale_item_receipt_line_sort_order_update($sale_id,$item_id, $item_class,$receipt_line_sort_order){
		if($item_class=="item"){
			$this->db->where('sale_id', $sale_id);
			$this->db->where('item_id', $item_id);
			$this->db->update('sales_items',array('receipt_line_sort_order'=>$receipt_line_sort_order));
		}else if($item_class="item-kit"){
			$this->db->where('sale_id', $sale_id);
			$this->db->where('item_kit_id', $item_id);
			$this->db->update('sales_item_kits',array('receipt_line_sort_order'=>$receipt_line_sort_order));
		}
		return true;
	}	
	
    public function is_open_suspended_sale($sale_id)
    {
        $cur_employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        ...
        $this->db->from('open_suspended_sales');
        $this->db->where('sale_id', $sale_id);
        $this->db->where('employee_id !=', $cur_employee_id);
        $this->db->where('expires > ', $cur_date_time);
        $query = $this->db->get();
        
        return ($query->num_rows() == 1);
    }
    
    public function get_opened_sale_info($sale_id)
    {
        $this->db->select('open_suspended_sales.*, people.full_name as employee_name, registers.name as register_name');
        $this->db->from('open_suspended_sales');
        $this->db->join('people', 'people.person_id = open_suspended_sales.employee_id', 'left');
        $this->db->join('registers', 'registers.register_id = open_suspended_sales.register_id', 'left');
        $this->db->where('sale_id', $sale_id);
        return $this->db->get()->row();
    }
    
    public function save_open_suspended_sale($sale_id)
    {
        ...
        $open_suspended_sale_data = array(
            'sale_id' => $sale_id,
            'employee_id' => $this->Employee->get_logged_in_employee_info()->person_id,
            'register_id' => $this->Employee->get_logged_in_employee_current_register_id() ?: $sale_info->register_id,
            'expires' => $expires,
        );
        
        $this->db->delete('open_suspended_sales', array('sale_id' => $sale_id));
        
        if ($this->db->insert('open_suspended_sales', $open_suspended_sale_data)) {
            return true;
        }
        return false;
    ...
    public function delete_open_suspended_sales($employee_id = false)
    {
        if (!$employee_id) {
            $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        }
        return $this->db->delete('open_suspended_sales', array('employee_id' => $employee_id));
    }

	function get_sale_item_info_by_sale_id_and_line($sale_id,$line)
	{
		$this->db->from('sales_items');
		$this->db->where('sale_id',$sale_id);
		$this->db->where('line',$line);
...
// dd($sales_item_data);
	...
	function sale_item_assigned_to_update($sale_id,$item_id,$line,$assigned_to,$is_item_kit){
		$sale_item = $this->get_sale_item($sale_id,$item_id,$line);
		if($is_item_kit) {
			$sale_item = $this->get_sale_item_kit($sale_id,$item_id,$line);
		}
		...
		if($sale_item){
			...
			$sales_item_data['assigned_to'] = $assigned_to;
	...
	function set_default_register_if_not_set($sale_id)
	{
		if (!$this->Employee->get_logged_in_employee_current_register_id())
		{
			...
			{
				$this->Employee->set_employee_current_register_id($sale_info->register_id);
			}
			else
			{
				$registers = $this->Register->get_all($sale_info->location_id);
	 ...
	 function get_sale_item_kit_info_by_sale_id_and_line($sale_id,$line)
	 {
		 $this->db->from('sales_item_kits');
		 $this->db->where('sale_id',$sale_id);
		 $this->db->where('line',$line);
	...
	function sale_item_kit_quantity_update($sale_id,$line,$quantity){
		$sale_item = $this->get_sale_item_kits($sale_id,$line)->row();
		...
		if($sale_item){
			...
			if(!$customer_id || $customer_info->taxable){
				$tax = $this->get_taxes_for_sale_item($sale_item->item_kit_id,$unit_price,$quantity);
			}
			else{
				$tax = 0;
			}
	...
	function get_max_sale_item_kits_line($sale_id)
	{
		$this->db->select('MAX('.$this->db->dbprefix('sales_item_kits').'.line) as max_line');
		$this->db->from('sales_item_kits');
		$this->db->where('sale_id',$sale_id);
		$this->db->group_by('sale_id');
		$query = $this->db->get();
	...
	function sale_item_kit_unit_price_update($sale_id,$item_kit_id,$line,$unit_price) {
		$sale_item = $this->get_sale_item_kit($sale_id,$item_kit_id);

		if($sale_item) {
	
			$item_info 		= $this->Item_kit->get_info($sale_item->item_kit_id);
			$cost_price 	= $sale_item->item_kit_cost_price;
			$quantity 		= $sale_item->quantity_purchased;
			$subtotal 		= $quantity * $unit_price;
	...
	function get_sale_item_kit($sale_id,$item_kit_id)
	{
		$this->db->from('sales_item_kits');
		$this->db->where('sale_id',$sale_id);
		$this->db->where('item_kit_id',$item_kit_id);
	...
	function sale_item_serialnumber_update($sale_id, $item_id, $line=0, $serialnumber=""){
		$sale_item = $this->get_sale_item($sale_id, $item_id, $line);
		if($sale_item){
			$variation_id = NULL;
	...
	function sale_item_discount_update($sale_id, $item_id, $line=0, $discount="", $is_item_kit = false){
		if($is_item_kit) {
			$sale_item 		= $this->get_sale_item_kit_info_by_sale_id_and_line($sale_id, $line);
			$search_field 	= 'item_kit_id';
			$update_table   = 'sales_item_kits';
		} else {
			$sale_item 		= $this->get_sale_item($sale_id, $item_id, $line);
			$search_field 	= 'item_id';
			$update_table   = 'sales_items';
		}
		...
		if($sale_item){
			...
			$this->db->where('sale_id', $sale_id);
			$this->db->where($search_field, $item_id);
			$this->db->where('line', $line);
	...
	*/
	...
	function get_stats_for_graph( $time ='all_time' , $from_date='' , $to_date = ''){
				...
				$query = 'SELECT COUNT(sales.sale_id) AS total, CONCAT(pep.first_name, " ", pep.last_name) AS full_name 
						...
						WHERE sales.is_work_order = 0 AND sales.location_id = ' . $location_id . ' ';
				...
				$query .= ' GROUP BY sales.employee_id ORDER BY COUNT(sales.sale_id) DESC LIMIT ' . (int)$top;
