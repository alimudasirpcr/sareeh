...
class Person extends MY_Model
{
	...
	*/
	...
	function get_titles()
	{
		$this->db->select('people_name_prefixes.*');
		$this->db->from('people_name_prefixes');
		$this->db->order_by('people_name_prefixes.id');
		return $this->db->get();
	}

	function search_title($search)
	{
		$title_list = $this->get_titles()->result_array();
		foreach($title_list as $index => $title){
			if($index <= 11){
				if($search == lang($title['name'])){
					return  true;
				}
			}else{
	...
	{
		$person_data['name'] = $title;
		if($this->db->insert('people_name_prefixes',$person_data)){
