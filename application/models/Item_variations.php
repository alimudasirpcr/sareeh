...
class Item_variations extends MY_Model
{
	...
	function get_all($item_id = false, $return_query = false)
	{
		$this->db->select('*');
		$this->db->from("item_variations");
	...
	*/
	...
	function get_attributes($item_variation_id = array() , $item_id = array())
	{
		...
		$this->db->dbprefix('item_variations').'.id as item_variation_id, people.full_name as supplier_name', FALSE);
	...
	function lookup($item_id, $attribute_value_ids, $supplier_id = false)
	{
		...
		$this->db->where("item_variations.item_id", $item_id);
		if($supplier_id){
			$this->db->where("item_variations.supplier_id", $supplier_id);
		}
	...
	function auto_create($item_id)
	{
		...
		foreach($variations as $variation)
		{
			...
			//if item has secondary supplier then add supplier at variation for first level supplier
			if($secondary_supplier_query->num_rows() >= 1){
				$variation_id = $this->lookup($item_id, $attribute_value_ids_for_variation, $item->supplier_id);
			
				$data = array('item_id' => $item_id, 'supplier_id' => $item->supplier_id);
			}else{
		...
		//if item has secondary supplier
		foreach($secondary_supplier_query->result() as $suppler){
			foreach($variations as $variation)
			{
				$attribute_value_ids_for_variation = array();
				...
				$variation_id = $this->lookup($item_id, $attribute_value_ids_for_variation, $suppler->supplier_id);
				
				$data = array('item_id' => $item_id, 'supplier_id' => $suppler->supplier_id);
