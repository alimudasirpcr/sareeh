	...
	public function getData()
{
    ...
    $this->db->where('receiving_time <=', $this->params['end_date']);
    $this->db->where('receivings.supplier_id', $this->params['supplier_id']);

    if ($this->params['receiving_type'] == 'receiving')
    {
        $this->db->where('receivings.total_quantity_purchased > 0');  // ✅ Fixed table name
    }
    elseif ($this->params['receiving_type'] == 'returns')
    {
        $this->db->where('receivings.total_quantity_purchased < 0');
    ...
    foreach ($result as $sale_summary_row) {
        $data['summary'][$sale_summary_row['receiving_id']] = $sale_summary_row; 
    }
