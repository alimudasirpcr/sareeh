...
class Summary_items_enhanced extends Report
{
	...
	{		
		...
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
		{
			...
			{
				$columns[] = array('data'=>$custom_field, 'align'=> 'right');
			}
		}
	...
	public function getOutputData()
	{
		...
		{				
			...
			if ($do_compare)
			{
				...
				$report_data_compare_model = new Summary_items_enhanced();
			...
			foreach($report_data as $row)
			{
				...
				if ($this->has_cost_price_permission)
				{
					$data_row[] = array('data'=>to_currency($row['avg_cost_price']), 'align' => 'right');
				}
				$data_row[] = array('data'=>to_currency($row['avg_selling_price']), 'align' => 'right');
				$data_row[] = array('data'=>$row['points_selling_price'], 'align' => 'right');
  			 	...
  			 	for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++) 
  				{
  					...
  					if($custom_field !== FALSE)
  					{
  						...
  						$data_row[] = array('data'=>$format_function($row["custom_field_${k}_value"]), 'align'=>'right');					
  					}
  				}				
	...
	private function get_items_with_sales_query($paginate,$only_item_id = FALSE)
	{
		...
		else
		{
			if (isset($this->params['group_by_variation']) && $this->params['group_by_variation'])
			{
				...
				$this->db->select('items.custom_field_1_value,items.custom_field_2_value,items.custom_field_3_value,items.custom_field_4_value,items.custom_field_5_value,items.custom_field_6_value,items.custom_field_7_value,items.custom_field_8_value,items.custom_field_9_value,items.custom_field_10_value,sales_items.item_variation_id as item_variation_id,items.item_id,suppliers.company_name as supplier,sales.location_id,items.cost_price as current_cost_price, items.unit_price as current_selling_price, avg('.$this->db->dbprefix('sales_items').'.item_cost_price) as avg_cost_price, avg('.$this->db->dbprefix('sales_items').'.item_unit_price) as avg_selling_price, group_concat(round('.$this->db->dbprefix('sales_items').'.item_unit_price, 2) SEPARATOR ", ") as points_selling_price, items.name, items.item_number, items.product_id, categories.name as category , items.category_id, sum('.$this->db->dbprefix('sales_items').'.quantity_purchased) as quantity_purchased, sum('.$this->db->dbprefix('sales_items').'.subtotal) as subtotal, sum('.$this->db->dbprefix('sales_items').'.total) as total, sum('.$this->db->dbprefix('sales_items').'.tax) as tax, sum('.$this->db->dbprefix('sales_items').'.profit) as profit, items.description as description', false);
			}
			else
			{
				$this->db->select('items.custom_field_1_value,items.custom_field_2_value,items.custom_field_3_value,items.custom_field_4_value,items.custom_field_5_value,items.custom_field_6_value,items.custom_field_7_value,items.custom_field_8_value,items.custom_field_9_value,items.custom_field_10_value,sales_items.item_variation_id as item_variation_id,items.item_id,suppliers.company_name as supplier,sales.location_id,items.cost_price as current_cost_price, items.unit_price as current_selling_price, avg('.$this->db->dbprefix('sales_items').'.item_cost_price) as avg_cost_price, avg('.$this->db->dbprefix('sales_items').'.item_unit_price) as avg_selling_price, group_concat(round('.$this->db->dbprefix('sales_items').'.item_unit_price, 2) SEPARATOR ", ") as points_selling_price, items.name, items.item_number, items.product_id, categories.name as category , items.category_id, sum('.$this->db->dbprefix('sales_items').'.quantity_purchased) as quantity_purchased, sum('.$this->db->dbprefix('sales_items').'.subtotal) as subtotal, sum('.$this->db->dbprefix('sales_items').'.total) as total, sum('.$this->db->dbprefix('sales_items').'.tax) as tax, sum('.$this->db->dbprefix('sales_items').'.profit) as profit, items.description as description', false);
	...
	private function get_items_without_sales($paginate = TRUE)
	{
		...
		if (isset($this->params['group_by_variation']) && $this->params['group_by_variation'])
		{
			...
			$this->db->select('items.custom_field_1_value,items.custom_field_2_value,items.custom_field_3_value,items.custom_field_4_value,items.custom_field_5_value,items.custom_field_6_value,items.custom_field_7_value,items.custom_field_8_value,items.custom_field_9_value,items.custom_field_10_value,item_variations.id as item_variation_id,items.item_id,suppliers.company_name as supplier,items.cost_price as current_cost_price,items.unit_price as current_selling_price,items.cost_price as avg_cost_price,items.unit_price as avg_selling_price, round('.$this->db->dbprefix('items').'.unit_price, 2) as points_selling_price, items.name, items.item_number, items.product_id, categories.name as category , items.category_id, 0 as quantity_purchased, 0 as subtotal, 0 as total, 0 as tax, 0 as profit, items.description as description', false);
		}
		else
		{
			$this->db->select('items.custom_field_1_value,items.custom_field_2_value,items.custom_field_3_value,items.custom_field_4_value,items.custom_field_5_value,items.custom_field_6_value,items.custom_field_7_value,items.custom_field_8_value,items.custom_field_9_value,items.custom_field_10_value,items.item_id,suppliers.company_name as supplier,items.cost_price as current_cost_price,items.unit_price as current_selling_price,items.cost_price as avg_cost_price,items.unit_price as avg_selling_price,round('.$this->db->dbprefix('items').'.unit_price,2) as points_selling_price, items.name, items.item_number, items.product_id, categories.name as category , items.category_id, 0 as quantity_purchased, 0 as subtotal, 0 as total, 0 as tax, 0 as profit, items.description as description', false);
		}
	...
	private function get_item_kits_without_sales($paginate = TRUE)
	{
		...
		if (isset($this->params['group_by_variation']) && $this->params['group_by_variation'])
		{
			$this->db->select('"" as item_variation_id,item_kits.item_kit_id,"" as supplier,item_kits.cost_price as current_cost_price,item_kits.unit_price as current_selling_price,item_kits.cost_price as avg_cost_price,item_kits.unit_price as avg_selling_price,item_kits.unit_price as points_selling_price, item_kits.name, item_kits.item_kit_number as item_number, item_kits.product_id, categories.name as category , item_kits.category_id, 0 as quantity_purchased, 0 as subtotal, 0 as total, 0 as tax, 0 as profit, item_kits.description as description', false);
		}
		else
		{
	...
	{		
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_without_sales' || $this->params['items_to_show'] == 'items_all'))
		{
			if (isset($this->params['include_item_kits']) && $this->params['include_item_kits'])
			{
		...
		elseif($this->params['items_to_show'] == 'items_all')
		{
			$return = array_merge($items_sales_data, $items_in_report_without_sales);
		}
		return $return;
	...
	function getTotalRows()
	{
		...
		elseif($this->params['items_to_show'] && $this->params['items_to_show'] == 'items_all')
		{
			return ($this->get_items_with_sales_query(FALSE)->count_all_results() + $this->get_items_without_sales(FALSE)->count_all_results());
		}
	}
	
	public function getSummaryData()
	{
		if($this->params['items_to_show'] == 'items_with_sales' || $this->params['items_to_show'] == 'items_all')
		{
			if ($this->params['category_id'])
			{
				if ($this->config->item('include_child_categories_when_searching_or_reporting'))
