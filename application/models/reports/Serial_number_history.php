...
class Serial_number_history extends Report
{
	...
	{		
		...
		$query = "SELECT 'receiving' as type, receiving_id as id,receiving_time as action_date FROM phppos_receivings INNER JOIN phppos_receivings_items USING (receiving_id) WHERE TRIM(TRAILING '\n' FROM serialnumber) = $serial_number UNION ALL SELECT 'sale' as type,sale_id as id,sale_time as action_date FROM phppos_sales INNER JOIN phppos_sales_items USING (sale_id) WHERE TRIM(TRAILING '\n' FROM phppos_sales_items.serialnumber)=$serial_number ORDER BY action_date";
