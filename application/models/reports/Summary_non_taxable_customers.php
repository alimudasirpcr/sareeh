...
class Summary_non_taxable_customers extends Report
{
	...
	{
		$this->db->select('COUNT(*) as count,customers.tax_certificate,customers.current_sales_for_discount, customer_id, CONCAT(first_name, " ",last_name) as customer, customers.person_id as person_id, people.phone_number, people.email, people.zip,people.address_1,people.address_2,people.house_no,people.state,people.city, sum(total) as total,sum(profit) as profit, sum(total_quantity_purchased) as total_quantity_purchased', false);
		$this->db->from('sales');
