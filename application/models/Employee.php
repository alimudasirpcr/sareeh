...
class Employee extends Person
{
	...
	*/
	...
	function create_sso_employee($data)
	{
		$person_data = array(
		...
		{
			$employee_data['template_id'] = $data['template_id'];
		}
		
		$location_data = !empty($data['locations']) ? $data['locations'] : array(1);
		
		$empty = array();
		
		$this->Employee->save_employee($person_data,$employee_data,$empty, $empty, $location_data);
		
		if (isset($data['template_id']))
		{
			$this->Employee->update_permissions_based_on_assigned_template($employee_data['person_id']);
		}
	}
	
	function update_permissions_based_on_assigned_template($employee_id)
	{
		$template_id = $this->get_info($employee_id)->template_id;
		$this->db->delete('permissions', array('person_id' => $employee_id));
		$this->db->delete('permissions_actions', array('person_id' => $employee_id));
		
		$this->db->query("INSERT INTO phppos_permissions (person_id,module_id) SELECT '$employee_id', module_id FROM phppos_permissions_template WHERE template_id='$template_id'");
		$this->db->query("INSERT INTO phppos_permissions_actions (person_id,module_id,action_id) SELECT '$employee_id', module_id,action_id FROM phppos_permissions_template_actions WHERE template_id='$template_id'");
	}
	...
	*/
	...
	function get_item_columns_to_display()
	{
		...
		static $has_see_item_quantity_permission;
		
		if (!$has_see_item_quantity_permission)
		{
			$has_see_item_quantity_permission = $this->has_module_action_permission('items','see_item_quantity', $this->get_logged_in_employee_info()->person_id);
		}
		...
		if (!$has_see_item_quantity_permission)
		{
			if (isset($columns_to_display['quantity']))
			{
				unset($columns_to_display['quantity']);
			}
		}
	...
	function get_custom_field($number,$key="name")
	{
		...
		if (!$config_data)
		{
			$config_data = $this->config->item('employee_custom_field_prefs') ? unserialize($this->config->item('employee_custom_field_prefs')) : array();
		}
	...
	function get_displayable_columns()
	{
		...
		$columns = array(
			...
			'language' => 											array('sort_column' => 'language','label' => lang('language'),'format_function' => 'ucwords'),
			'commission_percent' => 						array('sort_column' => 'commission_percent','label' => lang('commission_default_rate'),'format_function' => 'to_quantity'),
			...
			'hourly_pay_rate' => 								array('sort_column' => 'hourly_pay_rate','label' => lang('hourly_pay_rate'),'format_function' => 'to_currency'),			
	...
	function approve_time_off($id)
	{
		...
		$this->db->where('id',$id);
		return $this->db->update('employees_time_off',array('approved' => 1,'deleted' => 0,'employee_approved_person_id' => $this->get_logged_in_employee_info()->person_id));
	}
	...
	function datatable_language(){
	    ...
	    "sInfoFiltered" =>   "(".lang('filtered')." ".lang('from')." _MAX_ ".lang('total')." ".lang('entries').")",
	    //"sInfoPostFix" =>    "",
	    //"sThousands" =>      ",",
	...
	function get_logged_employees_locations()
	{	
		$employee_id 			= $this->session->userdata('person_id');
		$employees_locations 	= $this->get_authenticated_location_ids($employee_id);

		foreach ($employees_locations as $key => $location) {
			$this->db->from('locations');
			$this->db->where('location_id', $location);
			$data[$key] = $this->db->get()->row();
		}
		return $data;
	}
	

	function get_invoice_columns_to_display($invoice_type)
	{
		$all_columns = $this->Invoice->get_displayable_columns($invoice_type);
		
		$columns_to_display = array();
		
		$this->load->model('Employee_appconfig');
		if ($choices = $this->Employee_appconfig->get($invoice_type.'_invoices_column_prefs'))
		{
			$columns_to_display_keys = unserialize($choices);
		}
		else
	...
	function get_authenticated_locations($employee_id)
	{
		$this->db->select('locations.location_id,locations.name');
		$this->db->from('employees_locations');
		$this->db->join('locations', 'locations.location_id = employees_locations.location_id');
		$this->db->where('employee_id', $employee_id);
		$this->db->where('deleted', 0);
