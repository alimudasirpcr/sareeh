...
function get_item_kits_barcode_data($item_kits_ids, $predefined_barcode=false, $custom_fields = false, $estimated_repair_date = false)
{
	...
	foreach ($item_kit_ids as $item_kit_id)
	{
		...
		if ($id_to_show_on_barcode = $CI->config->item('id_to_show_on_barcode'))
		{
			...
			{
				$barcode_number = $item_kit_info->item_kit_number ? $item_kit_info->item_kit_number : ('KIT '.number_pad($item_kit_id,$CI->config->item('zerofill_barcode') ? $CI->config->item('zerofill_barcode') : 10));
			}
			elseif($id_to_show_on_barcode == 'product_id')
			{
				$barcode_number = $item_kit_info->product_id ? $item_kit_info->product_id : ('KIT '.number_pad($item_kit_id,$CI->config->item('zerofill_barcode') ? $CI->config->item('zerofill_barcode') : 10));
			}
		}
		
		$item_kit_location_info = $CI->Item_kit_location->get_info($item_kit_id);
		
		$item_kit_price = (double)$item_kit_location_info->unit_price ? $item_kit_location_info->unit_price : $item_kit_info->unit_price;
		...
		if($CI->config->item('barcode_price_include_tax'))
		{
			...
			{				
				$result[] = array('description' => $item_kit_info->description, 'name' =>(!$hide_prices ? '<span style="font-size:10pt;font-weight:bold;">'.to_currency(get_price_for_item_kit_including_taxes($item_kit_id,$item_kit_price)).'</span> ': '').($CI->config->item('hide_name_on_barcodes') ? '' : ($item_kit_info->barcode_name ? $item_kit_info->barcode_name : $item_kit_info->name)), 'id'=> $barcode_number);
	  	 	}
	  }
	  else
	  {
		if ($item_kit_info->tax_included)
		{
		    $result[] = array('description' => $item_kit_info->description, 'name' =>(!$hide_prices ? '<span style="font-size:10pt;font-weight:bold;">'.to_currency(get_price_for_item_kit_excluding_taxes($item_kit_id, $item_kit_price)).'</span> ' : '').($CI->config->item('hide_name_on_barcodes') ? '' : ($item_kit_info->barcode_name ? $item_kit_info->barcode_name : $item_kit_info->name)), 'id'=> $barcode_number);
		}
		else
		{
	    	$result[] = array('description' => $item_kit_info->description, 'name' =>(!$hide_prices ? '<span style="font-size:10pt;font-weight:bold;">'.to_currency($item_kit_price).'</span> ' : '').($CI->config->item('hide_name_on_barcodes') ? '' : ($item_kit_info->barcode_name ? $item_kit_info->barcode_name : $item_kit_info->name)), 'id'=> $barcode_number);
